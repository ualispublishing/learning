#!/usr/bin/env python3
"""Record fresh Arabic Gate B C1 Unit 2 decisions."""
...
DECISION=READING/'audit/arabic_gate_b_decisions_2026-08-30/c1_u02.json'
IDS=[f'ar-c1-u02-p{i:02d}' for i in range(1,7)]; FINDINGS=8; WITH=6
def sha(b): return hashlib.sha256(b).hexdigest()
def meta():
 p=Path(__file__).with_name('apply_arabic_gate_b_c1_unit02.py'); s=importlib.util.spec_from_file_location('c1u02',p); m=importlib.util.module_from_spec(s); s.loader.exec_module(m); return m.FINDING_META
def main():
 raw=CANONICAL.read_bytes(); rows=[json.loads(x) for x in raw.decode().splitlines() if x.strip()]; csha=sha(raw)
 if len(rows)!=60 or [rows[i].get('id') for i in range(6,12)]!=IDS: raise SystemExit('C1 Unit 2 layout/id drift')
 for r in rows[6:12]:
  q=r.get('quality',{})
