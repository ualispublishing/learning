...
STAGE = ROOT / "reading" / "audit" / "urdu_b1_u04_generation_candidate"
TARGET = ROOT / "reading" / "urdu" / "b1" / "passages.jsonl"
SCHEMA = ROOT / "reading" / "schema" / "passage.schema.json"
DATE = "2026-08-24"

EXPECTED_IDS = [f"ur-b1-u04-p{i:02d}" for i in range(1, 7)]
EXPECTED_SEQ = list(range(19, 25))
EXPECTED_TARGETS = [
    "ur-rank-1603","ur-rank-1608","ur-rank-1600",
    "ur-rank-1557","ur-rank-1639","ur-rank-1543",
    "ur-rank-1593","ur-rank-1559","ur-rank-1563",
    "ur-rank-1564","ur-rank-1541","ur-rank-1644",
    "ur-rank-1619","ur-rank-1613","ur-rank-1548",
]
EXPECTED_ROLES = ["instructional","reinforcement","interleaved","transfer","integration","checkpoint"]
REQUIRED_GENRES = {"report","problem-solution article","viewpoint"}
...
def main() -> int:
    ...
    if len(b1) != 18 or [r.get("sequence") for r in b1] != list(range(1,19)):
        fail("Urdu B1 must contain exactly sequences 1-18 before Unit 4 promotion")
    ...
    if status["current"]["canonical_passages"] != 858 or status["languages"]["urdu"]["canonical_passages"] != 138:
        fail("STATUS counts are no longer the reviewed Unit 4 pre-promotion state")
    ...
    if plan.get("active_unit") != 4 or plan.get("start_sequence") != 19 or plan.get("existing_active_level_passages") != 18:
        fail("ACTIVE_GENERATION_PLAN no longer points to Urdu B1 Unit 4 / sequence 19")
    action = continuation.get("active_frontier",{}).get("production",{}).get("action","")
    if "Urdu B1 Unit 4 / sequence 19" not in action:
        fail("CONTINUATION frontier no longer points to Unit 4 / sequence 19")
    ...
    for i in range(1,7):
        path = STAGE / f"ur-b1-u04-p{i:02d}.json"
        if not path.exists():
    ...
    if review_forms != expected_forms or any(form not in p6["text"] for form in expected_forms):
        fail("Unit 4 P6 does not visibly recycle all 15 new targets")
    ...
    (STAGE / "manifest.json").write_text(json.dumps({
        "schema_version":1,"project_id":"LANG-A1C2","language":"ur","cefr":"B1","unit":4,
        "date":DATE,"status":"CANONICALIZED","canonical_target":"reading/urdu/b1/passages.jsonl",
        "sequence_range":[19,24],"record_count":6,"release_promotion":False
    ...
    validation = {
        ...
        "checks":{
            "prior_a1_a2_60_each":"PASS","prior_b1_sequences_1_through_18_exact":"PASS",
            "freshness_across_all_prior_urdu_target_ids":"PASS","record_count":"PASS",
            "sequence_19_through_24":"PASS","role_cycle":"PASS","question_answer_10x10":"PASS",
    ...
    (ROOT / "reading" / "audit" / "urdu_b1_u04_generation_validation_2026-08-24.json").write_text(
    ...
    promotion = {
        ...
        "before_record_count":18,"after_record_count":24,
    ...
    (ROOT / "reading" / "audit" / "urdu_b1_u04_promotion_2026-08-24.json").write_text(
        json.dumps(promotion, ensure_ascii=False, indent=2) + "\n", encoding="utf-8"
    )

    lex_path = ROOT / "reading" / "audit" / "urdu_b1_u04_lexical_sense_check_2026-08-24.json"
    ...
    lex["freshness_scope"] = "Exact target IDs checked against all canonical Urdu A1, A2, and B1 sequences 1-18 immediately before Unit 4 append."
    lex_path.write_text(json.dumps(lex, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")

    status["current"]["canonical_passages"] = 864
    status["current"]["remaining_generation_passages"] = 216
    u = status["languages"]["urdu"]
    u["generation_state"] = "B1_IN_PROGRESS"
    u["canonical_passages"] = 144
    u["remaining_generation_passages"] = 216
    u["next_generation_level"] = "B1"
    status_path.write_text(json.dumps(status, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")

    continuation["production"]["canonical_passages"] = 864
    cu = continuation["production"]["urdu"]
    cu["state"] = "B1_GENERATION_IN_PROGRESS"
    cu["canonical_passages"] = 144
    cu["next_generation_level"] = "B1"
    continuation["active_frontier"]["production"] = {
        "language":"urdu","level":"B1",
        "action":"Continue generation-first production from Urdu B1 Unit 5 / sequence 25 using the canonical roadmap and ten-question contract."
    }
    continuation["exact_next_actions"] = [
        "Validate the routed state bundle and live canonical counts.",
        "Use reading/planning/ACTIVE_GENERATION_PLAN.json to start guarded Urdu B1 Unit 5 generation at sequence 25.",
    ...
    plan["active_unit_roadmap"] = {
        "unit":5,"theme":"health, habits, and evidence",
        "genres":["popular-science explanation","case story","advice critique"]
    }
    plan_path.write_text(json.dumps(plan, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")

    tasks_path = ROOT / "reading" / "TASKS.md"
    tasks = tasks_path.read_text(encoding="utf-8")
    tasks = replace_once(tasks, "Canonical production frontier: **Urdu B1, Unit 4, sequence 19**.",
                         "Canonical production frontier: **Urdu B1, Unit 5, sequence 25**.", "TASKS frontier")
    tasks = replace_once(tasks, "- Urdu: 138/360 generated; A1-A2 complete, B1 in progress.",
                         "- Urdu: 144/360 generated; A1-A2 complete, B1 in progress.", "TASKS Urdu total")
    tasks = replace_once(tasks, "- Project: 858/1080 generated.",
                         "- Project: 864/1080 generated.", "TASKS project total")
    ...
    h = replace_once(h, "- Canonical generated total: **858**.", "- Canonical generated total: **864**.", "handoff total")
    h = replace_once(h, "- Urdu: **138/360**; A1-A2 generation complete and B1 generation in progress.",
                     "- Urdu: **144/360**; A1-A2 generation complete and B1 generation in progress.", "handoff Urdu")
    h = replace_once(h, "Continue **Urdu B1**, starting from Unit 4 / sequence 19, under:",
                     "Continue **Urdu B1**, starting from Unit 5 / sequence 25, under:", "handoff frontier")
    h = replace_once(h,
                     "B1 Unit 4 uses the roadmap theme **environment and local decisions** with `report`, `problem-solution article`, and `viewpoint` genres.",
                     ...
                     "handoff roadmap")
    h = replace_once(h,
                     "resume guarded generation at **Urdu B1 Unit 4 / sequence 19** using the B1 Unit 4 roadmap theme `environment and local decisions`.",
    ...
    print("Unit 4 canonical promotion prepared: 18 -> 24 B1 records")
    print("Production totals prepared: 858 -> 864 project; Urdu 138 -> 144")
    print("Next frontier prepared: Urdu B1 Unit 5 / sequence 25")
    return 0
