#!/usr/bin/env python3
"""Guarded Pass 07 length remediation for Arabic A1 Unit 04."""
...
SPECS={
 'ar-a1-u04-p01':{'old_wc':84,'sentence_increment':2,'addition':'تنظر مريم إلى امرأة تقف قرب ليلى وتسأل عنها. تشرح ليلى أنها خالتها، ثم تشير إلى طفلة صغيرة وتذكر أنها قريبة لهم.'},
 'ar-a1-u04-p02':{'old_wc':89,'sentence_increment':2,'addition':'بعد الاستراحة تذهب نور مع ليلى إلى المكتبة الصغيرة في المدرسة. تختاران كتابًا قصيرًا، ثم تعودان إلى الصف قبل بدء الدرس.'},
}
...
def main():
 ...
 if set(SPECS)-set(by): raise SystemExit('missing Unit04 target')
 touched=[]
 for pid,s in SPECS.items():
  r=by[pid]; old=str(r.get('text','')).strip(); stored=int(r.get('word_count',0) or 0); actual=wc(old)
  if r.get('unit')!=4: raise SystemExit(f'{pid}: wrong unit')
  if stored!=s['old_wc'] or actual!=s['old_wc']: raise SystemExit(f'{pid}: source changed: {stored}/{actual}')
  ...
  if not 90<=r['word_count']<=140: raise SystemExit(f'{pid}: count {r["word_count"]} outside band')
  if target_counts(r)!=bt: raise SystemExit(f'{pid}: target occurrence changed')
  if json.dumps(r.get('questions'),ensure_ascii=False,sort_keys=True)!=bq: raise SystemExit(f'{pid}: questions changed')
  if json.dumps(r.get('answer_key'),ensure_ascii=False,sort_keys=True)!=ba: raise SystemExit(f'{pid}: answers changed')
 ...
 for pid in ('ar-a1-u04-p03','ar-a1-u04-p04','ar-a1-u04-p05','ar-a1-u04-p06'):
  if int(by[pid].get('word_count',0) or 0)<90: raise SystemExit(f'{pid}: expected already standard')
 PATH.write_text('\n'.join(json.dumps(r,ensure_ascii=False,sort_keys=True) for r in rows)+'\n',encoding='utf-8')
 print(json.dumps({'level':'A1','unit':4,'passages_touched':len(touched),'word_counts':touched},ensure_ascii=False))
