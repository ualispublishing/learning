#!/usr/bin/env python3
"""Resolve canonical French C1 Unit02 from the exact levels.C1 branch."""
...
def main():
 ...
 for i,node in enumerate(levels['C1']):
  ...
  if str(unit).strip().lower() in {'2','02','u02','unit02','unit 02','unit 2'}:matches.append((i,node))
 if len(matches)!=1:raise AssertionError(f'Expected exactly one levels.C1 Unit02 node, found {len(matches)}')
 ...
 if not isinstance(theme,str) or not theme.strip():raise AssertionError(f'C1 Unit02 node lacks explicit theme at {path}')
 out={'status':'PASS','scope':'French C1 Unit02 canonical planning resolution','b2_canonical_blob':b2blob,'c1_source_blob':c1blob,'c1_unit01_lock':'reading/audit/french_c1_unit01_frontier_lock.json','c1_word_min':ready['c1_word_min'],'c1_word_max':ready['c1_word_max'],'matrix_path':path,'theme':theme,'genres':genres,'domains':domains,'notes':notes,'canonical_node':node,'accepted_c1_default_new_targets_per_standard_passage':lock['accepted_c1_default_new_targets_per_standard_passage'],'accepted_default_is_hard_quota':False,'note':'Resolved from exact canonical levels.C1 Unit02 node after calibrated Unit01 lock.'}
