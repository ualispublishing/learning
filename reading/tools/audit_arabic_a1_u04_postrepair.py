...
PATH=ROOT/'reading/arabic/a1/passages.jsonl'; OUT=ROOT/'reading/audit/arabic_a1_u04_metalinguistic_postrepair_2026-08-20.json'; EXPECTED='62036a705af9db459f382482f394487decf3cc49117d1d9497bafa862e373df5'
...
def main():
 ...
 rows=[json.loads(x) for x in PATH.read_text(encoding='utf-8').splitlines() if x.strip()]; unit=[p for p in rows if p.get('unit')==4]
 if len(unit)!=6 or [p.get('sequence') for p in unit]!=list(range(19,25)): raise SystemExit('scope regression')
 ...
 if out['status']!='PASS_DETERMINISTIC_UNIT04': raise SystemExit('postrepair audit failed')
