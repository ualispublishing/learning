...
At exactly 12 canonical C1 passages, verify the existing Unit02 lock/review and
prepare Unit03 directly; do not replay Unit01/Unit02 source-prefix planning. When
later units exist, verify the sealed first-twelve prefix and exit read-only.
"""
...
def verify_unit02_blob(blob):
 lock=json.loads((AUD/'french_c1_unit02_frontier_lock.json').read_text(encoding='utf-8'));review=json.loads((AUD/'french_c1_unit02_generation_review.json').read_text(encoding='utf-8'));b2blob=h(B2)
 if lock.get('status')!='PASS' or lock.get('last_sequence')!=12 or lock.get('c1_canonical_blob')!=blob or lock.get('b2_canonical_blob')!=b2blob:raise AssertionError('sealed C1 Unit02 lock mismatch')
 if review.get('status')!='PASS' or review.get('c1_canonical_blob')!=blob:raise AssertionError('C1 Unit02 review/blob mismatch')
 return lock,b2blob
def main():
 ...
 if before_n>12:
  pblob=prefix_blob(12);lock,b2blob=verify_unit02_blob(pblob)
 ...
 if before_n==12:
  try:
   live=h(C1);lock,_=verify_unit02_blob(live);unit02_sealed=True;stages.append('verify_existing_sealed_c1_unit02')
   ...
   error=traceback.format_exc();print(error);(AUD/'french_c1_unit02_pipeline_failure.txt').write_text(error,encoding='utf-8')
  if frontier_pass:
   for p in AUD.glob('french_c1_unit02_*failure.txt'):p.unlink(missing_ok=True)
  status='PASS_TO_C1_UNIT03' if frontier_pass else 'C1_UNIT02_PASS_UNIT03_PREP_PENDING';result={'status':status,'date':'2026-08-17','starting_c1_passages':12,'ending_c1_passages':12,'b2_blob':h(B2),'c1_blob':h(C1),'c1_unit02_pass':True,'c1_unit03_frontier_prepared':frontier_pass,'completed_stages':stages,'error':error,'accepted_c1_default':lock['accepted_c1_default_new_targets_per_standard_passage'],'default_is_hard_quota':False}
  if frontier_pass:
 ...
 try:
  ...
  else:raise AssertionError(f'unsupported C1 frontier for Unit02 generation: {len(rs)} rows')
  run('audit_french_c1_unit02_generation.py');stages.append('audit_c1_unit02_generation');review=json.loads((AUD/'french_c1_unit02_generation_review.json').read_text(encoding='utf-8'))
  if review.get('status')!='PASS' or review.get('c1_canonical_blob')!=h(C1):raise AssertionError('Unit02 review/live blob mismatch')
  run('lock_french_c1_unit02_frontier.py');stages.append('lock_c1_unit02');lock2=json.loads((AUD/'french_c1_unit02_frontier_lock.json').read_text(encoding='utf-8'))
  if lock2.get('status')!='PASS' or lock2.get('c1_canonical_blob')!=h(C1):raise AssertionError('Unit02 lock/live mismatch')
  unit02_sealed=True;run('resolve_french_c1_unit03_plan.py');stages.append('resolve_c1_unit03_plan');run('probe_french_c1_unit03_targets.py');stages.append('probe_c1_unit03_targets');run('sync_french_c1_unit02_frontier.py');stages.append('sync_c1_unit02_to_unit03');frontier_pass=True
 except Exception:
 ...
 if frontier_pass:
  for p in AUD.glob('french_c1_unit02_*failure.txt'):p.unlink(missing_ok=True)
 status='PASS_TO_C1_UNIT03' if frontier_pass else ('C1_UNIT02_PASS_UNIT03_PREP_PENDING' if unit02_sealed else 'C1_UNIT02_PENDING');result={'status':status,'date':'2026-08-17','starting_c1_passages':before_n,'ending_c1_passages':len(rows()),'b2_blob':h(B2),'c1_blob':h(C1),'c1_unit02_pass':unit02_sealed,'c1_unit03_frontier_prepared':frontier_pass,'completed_stages':stages,'error':error}
 if unit02_sealed:
