...
QA = ROOT / "reading/audit/urdu_c2_u03_generation_qa_2026-08-29.json"
SPECS_DIR = ROOT / "reading/tools/c2_unit03_specs"

UNIT = 3
START_SEQUENCE = 13
EXPECTED_EXISTING = 12
EXPECTED_PROJECT = 1032
EXPECTED_URDU = 312
NEW_PROJECT = 1038
NEW_URDU = 318
NEW_REMAINING = 42
NEXT_UNIT = 4
NEXT_SEQUENCE = 19
THEME = "science, models, and epistemic limits"
GENRES = ["advanced synthesis", "critique", "research commentary"]
NEXT_THEME = "economics and complex systems"
NEXT_GENRES = ["analytical essay", "scenario analysis", "commentary"]
...
def make_record(spec, idx):
    ...
    return {
        ...
        "grammar_targets": [{
            "id": "ur-c2-u03-grammar-01",
            "role": "integration",
            "description": "Use conditional, concessive, evidential, causal, and scope-sensitive structures to distinguish observations, assumptions, model claims, measurement limits, and justified conclusions."
        }],
        "discourse_targets": [{
            "id": "ur-c2-u03-discourse-01",
            "role": "integration",
            "description": "Track model assumptions, validation boundaries, falsifiability, generalizability, measurement uncertainty, alternative explanations, and calibrated confidence across advanced scientific argument."
        }],
        "questions": questions,
        "answer_key": answers,
        "speed_training": {
        ...
        "quality": {
            ...
            "notes": [
                ...
                "All studies, instruments, institutions, datasets, models, and scenarios are explicitly hypothetical instructional constructs rather than factual scientific claims."
            ]
        },
        "reader_tags": ["C2", "science", "models", "epistemic limits", "synthetic scenario"],
...
def validate_existing(existing, cont, status, plan):
    ...
    if [p.get("sequence") for p in existing] != list(range(1, EXPECTED_EXISTING + 1)):
        raise SystemExit("Existing Urdu C2 sequence is not exactly 1..12")
    if existing[-1].get("id") != "ur-c2-u02-p06":
        raise SystemExit(f"Unexpected Urdu C2 tail id: {existing[-1].get('id')}")
    if cont["production"]["canonical_passages"] != EXPECTED_PROJECT or status["current"]["canonical_passages"] != EXPECTED_PROJECT:
        raise SystemExit("Project count drift")
    if cont["production"]["urdu"]["canonical_passages"] != EXPECTED_URDU or status["languages"]["urdu"]["canonical_passages"] != EXPECTED_URDU:
        raise SystemExit("Urdu count drift")
    if "C2 Unit 3 / sequence 13" not in cont["active_frontier"]["production"]["action"]:
    ...
    if roadmap.get("theme") != THEME or roadmap.get("genres") != GENRES:
        raise SystemExit("ACTIVE_GENERATION_PLAN Unit 3 roadmap drift")
...
def validate_new_records(records):
    ...
    if [r["passage_type"] for r in records] != expected_types:
        raise SystemExit("Unit 3 passage-role cycle drift")
    if [r["sequence"] for r in records] != list(range(13, 19)):
        raise SystemExit("Unit 3 sequences must be 13..18")
    if len({r["id"] for r in records}) != 6:
        raise SystemExit("Duplicate Unit 3 passage ids")
    for r in records:
    ...
    if records[-1]["new_lexical_targets"]:
        raise SystemExit("C2 Unit 3 checkpoint must introduce no new lexical target")
...
def update_state(cont, status, plan):
    ...
    cont["active_frontier"]["production"]["action"] = "Continue generation-first production from Urdu C2 Unit 4 / sequence 19 using the canonical roadmap and ten-question contract."
    cont["exact_next_actions"] = [
        "Validate the routed state bundle and live canonical counts.",
        "Use reading/planning/ACTIVE_GENERATION_PLAN.json to start guarded Urdu C2 Unit 4 generation at sequence 19.",
...
def update_markdown():
    ...
    tasks = replace_required(tasks, "Canonical production frontier: **Urdu C2, Unit 3, sequence 13**.", "Canonical production frontier: **Urdu C2, Unit 4, sequence 19**.", "TASKS frontier")
    tasks = replace_required(tasks, "- Urdu: 312/360 generated; A1-C1 complete, C2 in progress.", "- Urdu: 318/360 generated; A1-C1 complete, C2 in progress.", "TASKS Urdu total")
    tasks = replace_required(tasks, "- Project: 1032/1080 generated.", "- Project: 1038/1080 generated.", "TASKS project total")
    TASKS.write_text(tasks, encoding="utf-8")

    handoff = HANDOFF.read_text(encoding="utf-8")
    handoff = replace_required(handoff, "- Canonical generated total: **1032**.", "- Canonical generated total: **1038**.", "handoff project total")
    handoff = replace_required(handoff, "- Urdu: **312/360**; A1-C1 generation complete and C2 generation in progress.", "- Urdu: **318/360**; A1-C1 generation complete and C2 generation in progress.", "handoff Urdu total")
    handoff = replace_required(handoff, "Continue **Urdu C2**, starting from Unit 3 / sequence 13, under:", "Continue **Urdu C2**, starting from Unit 4 / sequence 19, under:", "handoff frontier")
    handoff = replace_required(handoff, "C2 Unit 3 uses the roadmap theme **science, models, and epistemic limits** with `advanced synthesis`, `critique`, and `research commentary` genres.", "C2 Unit 4 uses the roadmap theme **economics and complex systems** with `analytical essay`, `scenario analysis`, and `commentary` genres.", "handoff roadmap")
    handoff = replace_required(handoff, "Run `python reading/tools/validate_continuation_state.py`; if it passes, resume guarded generation at **Urdu C2 Unit 3 / sequence 13** using the C2 Unit 3 roadmap theme `science, models, and epistemic limits`.", "Run `python reading/tools/validate_continuation_state.py`; if it passes, resume guarded generation at **Urdu C2 Unit 4 / sequence 19** using the C2 Unit 4 roadmap theme `economics and complex systems`.", "handoff exact next action")
...
def main():
    ...
    spec_paths = sorted(SPECS_DIR.glob("p*.json"))
    if len(spec_paths) != 6:
        raise SystemExit(f"Expected 6 Unit 3 spec files, found {len(spec_paths)}")
    ...
    update_markdown()
    qa = {
        "project_id": "LANG-A1C2", "language": "urdu", "level": "C2", "unit": UNIT,
        "sequences": list(range(13, 19)), "passage_ids": [r["id"] for r in records],
        "theme": THEME, "genres": [r["genre"] for r in records],
        "canonicalization_status": "CANONICAL_APPEND_CONFIRMED", "canonical_passage_count": 18,
        "project_canonical_passages": NEW_PROJECT, "urdu_canonical_passages": NEW_URDU,
        ...
        "questions_per_passage": 10, "answers_per_passage": 10, "hard_errors": 0,
        "formal_release_audit": "DEFERRED", "release_claim": False,
        "next_frontier": "Urdu C2 Unit 4 / sequence 19",
        "notes": [
            "All six passages use explicitly hypothetical scientific studies, instruments, models, and institutions; no factual real-world scientific claim is asserted.",
            "Five specialist Unit 3 lexical targets are marked beyond_base rather than assigned invented frequency ranks; the checkpoint introduces no new lexical target.",
    ...
    subprocess.run([sys.executable, "reading/tools/validate_continuation_state.py"], cwd=ROOT, check=True)
    print(json.dumps({"status": "ok", "appended": 6, "urdu_c2_passages": 18, "project_passages": NEW_PROJECT, "next_frontier": "Urdu C2 Unit 4 / sequence 19"}))
