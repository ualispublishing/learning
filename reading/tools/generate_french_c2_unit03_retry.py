#!/usr/bin/env python3
"""Quality preflight for C2 Unit03: deepen the zero-new checkpoint and remove internal authoring language."""
from pathlib import Path

HERE = Path(__file__).resolve().parent
p = HERE / 'generate_french_c2_unit03.py'
ns = {'__name__': 'c2_u03_base', '__file__': str(p), '__package__': None}
exec(compile(p.read_text(encoding='utf-8'), str(p), 'exec'), ns)

# Keep learner-facing language distinct from internal generation terminology.
ns['USE']['terrain'] = (
    'Le « terrain » désigne ici le contexte empirique où un modèle rencontre des observations '
    'non produites pour lui; la performance hors du jeu de données d’ajustement constitue un test '
    'distinct de l’ajustement interne.'
)

_orig_specs = ns['specs']
def specs():
    rows = _orig_specs()
    for row in rows:
        row['paras'] = [
            para.replace('hors calibration', 'hors des données d’ajustement')
                .replace('jeu de calibration', 'jeu de données d’ajustement')
                .replace('domaine de calibration', 'domaine d’ajustement')
            for para in row['paras']
        ]
    return rows
ns['specs'] = specs
