...
QA = ROOT / "reading/audit/urdu_c2_u02_generation_qa_2026-08-29.json"
SPECS = ROOT / "reading/tools/unit02_c2_specs.json"

UNIT = 2
START_SEQUENCE = 7
EXPECTED_EXISTING = 6
EXPECTED_PROJECT = 1026
EXPECTED_URDU = 306
NEW_PROJECT = 1032
NEW_URDU = 312
NEW_REMAINING = 48
NEXT_UNIT = 3
NEXT_SEQUENCE = 13
THEME = "law and competing interpretations"
GENRES = ["dense analysis", "case commentary", "position comparison"]
NEXT_THEME = "science, models, and epistemic limits"
NEXT_GENRES = ["advanced synthesis", "critique", "research commentary"]
...
def make_record(spec, idx):
    ...
    return {
        ...
        "grammar_targets": [{
            "id": "ur-c2-u02-grammar-01",
            "role": "integration",
            "description": "Use scope-sensitive, concessive, conditional, exception, and purpose structures to compare textual, structural, purposive, and proportional interpretations."
        }],
        "discourse_targets": [{
            "id": "ur-c2-u02-discourse-01",
            "role": "integration",
            "description": "Track rival interpretations across text, context, exceptions, procedural purpose, limiting principles, counterexamples, and future-case consequences without treating any fictional rule as real law."
        }],
        "questions": questions,
        "answer_key": answers,
        "speed_training": {
        ...
        "quality": {
            ...
            "notes": [
                ...
                "All rules, institutions, disputes, clauses, procedures, and parties are explicitly hypothetical instructional constructs rather than statements of real law."
            ]
        },
        "reader_tags": ["C2", "law", "interpretation", "competing positions", "synthetic scenario"],
...
def validate_existing(existing, cont, status, plan):
    ...
    if [p.get("sequence") for p in existing] != list(range(1, EXPECTED_EXISTING + 1)):
        raise SystemExit("Existing Urdu C2 sequence is not exactly 1..6")
    if existing[-1].get("id") != "ur-c2-u01-p06":
    ...
    if "C2 Unit 2 / sequence 7" not in cont["active_frontier"]["production"]["action"]:
    ...
    if roadmap.get("theme") != THEME or roadmap.get("genres") != GENRES:
        raise SystemExit("ACTIVE_GENERATION_PLAN Unit 2 roadmap drift")
...
def validate_new_records(records):
    ...
    if [r["passage_type"] for r in records] != expected_types:
        raise SystemExit("Unit 2 passage-role cycle drift")
    if [r["sequence"] for r in records] != list(range(7, 13)):
        raise SystemExit("Unit 2 sequences must be 7..12")
    if len({r["id"] for r in records}) != 6:
        raise SystemExit("Duplicate Unit 2 passage ids")
    for r in records:
    ...
    if records[-1]["new_lexical_targets"]:
        raise SystemExit("C2 Unit 2 checkpoint must introduce no new lexical target")
...
def update_state(cont, status, plan):
    ...
    cont["active_frontier"]["production"]["action"] = (
        "Continue generation-first production from Urdu C2 Unit 3 / sequence 13 using the canonical roadmap and ten-question contract."
    )
    cont["exact_next_actions"] = [
        "Validate the routed state bundle and live canonical counts.",
        "Use reading/planning/ACTIVE_GENERATION_PLAN.json to start guarded Urdu C2 Unit 3 generation at sequence 13.",
...
def update_markdown():
    ...
    tasks = replace_required(tasks,
        ...
        "Canonical production frontier: **Urdu C2, Unit 3, sequence 13**.",
        "TASKS frontier")
    tasks = replace_required(tasks,
        "- Urdu: 306/360 generated; A1-C1 complete, C2 in progress.",
        "- Urdu: 312/360 generated; A1-C1 complete, C2 in progress.",
        "TASKS Urdu total")
    tasks = replace_required(tasks,
        "- Project: 1026/1080 generated.",
        "- Project: 1032/1080 generated.",
        "TASKS project total")
    ...
    handoff = replace_required(handoff,
        ...
        "- Canonical generated total: **1032**.",
        "handoff project total")
    handoff = replace_required(handoff,
        "- Urdu: **306/360**; A1-C1 generation complete and C2 generation in progress.",
        "- Urdu: **312/360**; A1-C1 generation complete and C2 generation in progress.",
        "handoff Urdu total")
    handoff = replace_required(handoff,
        "Continue **Urdu C2**, starting from Unit 2 / sequence 7, under:",
        "Continue **Urdu C2**, starting from Unit 3 / sequence 13, under:",
        "handoff frontier")
    handoff = replace_required(handoff,
        ...
        "C2 Unit 3 uses the roadmap theme **science, models, and epistemic limits** with `advanced synthesis`, `critique`, and `research commentary` genres.",
        "handoff roadmap")
    handoff = replace_required(handoff,
        ...
        "Run `python reading/tools/validate_continuation_state.py`; if it passes, resume guarded generation at **Urdu C2 Unit 3 / sequence 13** using the C2 Unit 3 roadmap theme `science, models, and epistemic limits`.",
...
def main():
    ...
    if len(specs) != 6:
        raise SystemExit(f"Expected 6 C2 Unit 2 specs, found {len(specs)}")
    ...
    qa = {
        ...
        "release_claim": False,
        "next_frontier": "Urdu C2 Unit 3 / sequence 13",
        "notes": [
            "All six passages use explicitly hypothetical legal and institutional scenarios; no rule is presented as real-world law.",
            "Five specialist Unit 2 lexical targets are marked beyond_base rather than assigned invented frequency ranks; the checkpoint introduces no new lexical target.",
