...
STAGE = ROOT / "reading" / "audit" / "urdu_b2_u02_generation_candidate"
TARGET = ROOT / "reading" / "urdu" / "b2" / "passages.jsonl"
SCHEMA = ROOT / "reading" / "schema" / "passage.schema.json"
DATE = "2026-08-24"

EXPECTED_IDS = [f"ur-b2-u02-p{i:02d}" for i in range(1, 7)]
EXPECTED_SEQ = list(range(7, 13))
EXPECTED_TARGETS = [
    "ur-rank-2099", "ur-rank-2082",
    "ur-rank-2102", "ur-rank-2116",
    "ur-rank-2073", "ur-rank-2250",
    "ur-rank-2087", "ur-rank-2276",
    "ur-rank-2281", "ur-rank-2055",
]
EXPECTED_ROLES = ["instructional", "reinforcement", "interleaved", "transfer", "integration", "checkpoint"]
REQUIRED_GENRES = {"explanatory article", "case study", "commentary"}
...
def main() -> int:
    ...
    if len(b2) != 6 or [r.get("sequence") for r in b2] != list(range(1, 7)):
        fail("Urdu B2 must contain exactly sequences 1-6 before Unit 2 promotion")
    ...
    if status["current"]["canonical_passages"] != 906 or status["languages"]["urdu"]["canonical_passages"] != 186:
        fail("STATUS counts are no longer the reviewed B2 Unit 2 pre-promotion state")
    if status["current"]["active_language"] != "urdu" or status["current"]["active_level"] != "B2":
        fail("STATUS active frontier is no longer Urdu B2")
    if plan.get("active_unit") != 2 or plan.get("start_sequence") != 7 or plan.get("existing_active_level_passages") != 6:
        fail("ACTIVE_GENERATION_PLAN no longer points to Urdu B2 Unit 2 / sequence 7")
    action = continuation.get("active_frontier", {}).get("production", {}).get("action", "")
    if "Urdu B2 Unit 2 / sequence 7" not in action:
        fail("CONTINUATION frontier no longer points to Urdu B2 Unit 2 / sequence 7")
    ...
    if collision:
        fail(f"Unit 2 target-ID freshness collision: {collision}")

    rows: list[dict] = []
    learner_parts: list[str] = []
    for i in range(1, 7):
        path = STAGE / f"ur-b2-u02-p{i:02d}.json"
        if not path.exists():
    ...
    final = jl(TARGET)
    if len(final) != 12 or [r["sequence"] for r in final] != list(range(1, 13)) or [r["id"] for r in final[-6:]] != EXPECTED_IDS:
        fail("canonical B2 post-append check failed")

    (STAGE / "manifest.json").write_text(json.dumps({
        "schema_version": 1, "project_id": "LANG-A1C2", "language": "ur", "cefr": "B2", "unit": 2,
        "date": DATE, "status": "CANONICALIZED", "canonical_target": "reading/urdu/b2/passages.jsonl",
        "sequence_range": [7, 12], "record_count": 6, "release_promotion": False,
    }, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")

    checks = {k: "PASS" for k in [
        "prior_a1_a2_b1_60_each", "prior_b2_sequences_1_through_6_exact", "freshness_across_all_prior_urdu_target_ids",
        "record_count", "sequence_7_through_12", "role_cycle", "question_answer_10x10", "bidirectional_links",
        "new_target_distribution_2_2_2_2_2_0", "new_target_text_exposure", "first_introduction_order_unicode_word_boundary",
        "required_genres", "p6_checkpoint_policy", "p6_all_target_recycling", "learner_script_scan",
        "schema_required_fields_enums_context_and_review_metadata", "b2_word_band_350_550", "preexisting_canonical_bytes_preserved",
    ]}
    (ROOT / "reading" / "audit" / "urdu_b2_u02_generation_validation_2026-08-24.json").write_text(json.dumps({
        "schema_version": 1, "project_id": "LANG-A1C2", "language": "ur", "cefr": "B2", "unit": 2,
        "date": DATE, "canonicalized": True, "release_promotion": False,
        "word_counts": [r["word_count"] for r in rows], "checks": checks,
    }, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    (ROOT / "reading" / "audit" / "urdu_b2_u02_promotion_2026-08-24.json").write_text(json.dumps({
        "schema_version": 1, "project_id": "LANG-A1C2", "language": "ur", "cefr": "B2", "unit": 2,
        "date": DATE, "status": "CANONICAL_PROMOTION_PASS", "release_promotion": False,
        "before_record_count": 6, "after_record_count": 12, "appended_sequences": EXPECTED_SEQ, "appended_ids": EXPECTED_IDS,
    ...
    lexical_path = ROOT / "reading" / "audit" / "urdu_b2_u02_lexical_sense_check_2026-08-24.json"
    ...
    lexical["freshness_scope"] = "Exact target IDs checked against all canonical Urdu A1-B1 and B2 sequences 1-6 immediately before Unit 2 append."
    lexical_path.write_text(json.dumps(lexical, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")

    status["current"]["canonical_passages"] = 912
    status["current"]["remaining_generation_passages"] = 168
    urdu = status["languages"]["urdu"]
    urdu["generation_state"] = "B2_IN_PROGRESS"
    urdu["canonical_passages"] = 192
    urdu["remaining_generation_passages"] = 168
    urdu["next_generation_level"] = "B2"
    status_path.write_text(json.dumps(status, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")

    continuation["production"]["canonical_passages"] = 912
    cu = continuation["production"]["urdu"]
    cu["state"] = "B2_GENERATION_IN_PROGRESS"
    cu["canonical_passages"] = 192
    cu["next_generation_level"] = "B2"
    continuation["active_frontier"]["production"] = {
        "language": "urdu", "level": "B2",
        "action": "Continue generation-first production from Urdu B2 Unit 3 / sequence 13 using the canonical roadmap and ten-question contract.",
    }
    continuation["exact_next_actions"] = [
        "Validate the routed state bundle and live canonical counts.",
        "Use reading/planning/ACTIVE_GENERATION_PLAN.json to start guarded Urdu B2 Unit 3 generation at sequence 13.",
    ...
    plan["active_unit"] = 3
    plan["start_sequence"] = 13
    plan["canonical_active_path"] = "reading/urdu/b2/passages.jsonl"
    plan["existing_active_level_passages"] = 12
    plan["roadmap_lookup"] = "$.levels.B2"
    plan["active_unit_roadmap"] = {"unit": 3, "theme": "ethics and competing values", "genres": ["argument", "case", "response"]}
    plan_path.write_text(json.dumps(plan, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")

    tasks_path = ROOT / "reading" / "TASKS.md"
    tasks = tasks_path.read_text(encoding="utf-8")
    tasks = replace_once(tasks, "Canonical production frontier: **Urdu B2, Unit 2, sequence 7**.", "Canonical production frontier: **Urdu B2, Unit 3, sequence 13**.", "TASKS frontier")
    tasks = replace_once(tasks, "- Urdu: 186/360 generated; A1-B1 complete, B2 in progress.", "- Urdu: 192/360 generated; A1-B1 complete, B2 in progress.", "TASKS Urdu total")
    tasks = replace_once(tasks, "- Project: 906/1080 generated.", "- Project: 912/1080 generated.", "TASKS project total")
    tasks_path.write_text(tasks, encoding="utf-8")

    handoff_path = ROOT / "reading" / "AGENT_HANDOFF_V2.md"
    handoff = handoff_path.read_text(encoding="utf-8")
    handoff = replace_once(handoff, "- Canonical generated total: **906**.", "- Canonical generated total: **912**.", "handoff total")
    handoff = replace_once(handoff, "- Urdu: **186/360**; A1-B1 generation complete and B2 generation in progress.", "- Urdu: **192/360**; A1-B1 generation complete and B2 generation in progress.", "handoff Urdu")
    handoff = replace_once(handoff, "Continue **Urdu B2**, starting from Unit 2 / sequence 7, under:", "Continue **Urdu B2**, starting from Unit 3 / sequence 13, under:", "handoff frontier")
    handoff = replace_once(handoff, "B2 Unit 2 uses the roadmap theme **economics and personal/public choices** with `explanatory article`, `case study`, and `commentary` genres.", "B2 Unit 3 uses the roadmap theme **ethics and competing values** with `argument`, `case`, and `response` genres.", "handoff roadmap")
    handoff = replace_once(handoff, "resume guarded generation at **Urdu B2 Unit 2 / sequence 7** using the B2 Unit 2 roadmap theme `economics and personal/public choices`.", "resume guarded generation at **Urdu B2 Unit 3 / sequence 13** using the B2 Unit 3 roadmap theme `ethics and competing values`.", "handoff exact next")
    handoff_path.write_text(handoff, encoding="utf-8")

    print("Urdu B2 Unit 2 canonical promotion prepared: 6 -> 12 B2 records")
    print("Production totals prepared: 906 -> 912 project; Urdu 186 -> 192")
    print("Next frontier prepared: Urdu B2 Unit 3 / sequence 13")
    return 0
