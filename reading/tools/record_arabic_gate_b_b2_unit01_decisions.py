#!/usr/bin/env python3
"""Record fresh Arabic Gate B B2 Unit 1 decisions from the rebuilt Gate B inventory."""
...
def load_meta():
    source = Path(__file__).with_name("apply_arabic_gate_b_b2_unit01.py")
    spec = importlib.util.spec_from_file_location("arabic_gate_b_b2_u01_meta", source)
    if spec is None or spec.loader is None:
        raise SystemExit("cannot load B2 Unit 1 repair metadata")
...
def main() -> None:
    ...
    if (
        ...
        raise SystemExit("B2 Unit 1 finding metadata drift")
