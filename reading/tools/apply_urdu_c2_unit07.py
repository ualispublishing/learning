...
QA = ROOT / "reading/audit/urdu_c2_u07_generation_qa_2026-08-29.json"
SPECS_DIR = ROOT / "reading/tools/c2_unit07_specs"

UNIT = 7
START_SEQUENCE = 37
EXPECTED_EXISTING = 36
EXPECTED_PROJECT = 1056
EXPECTED_URDU = 336
NEW_PROJECT = 1062
NEW_URDU = 342
NEW_REMAINING = 18
NEXT_UNIT = 8
NEXT_SEQUENCE = 43
THEME = "culture, translation, and interpretation"
GENRES = ["essay", "comparative criticism", "reflection"]
NEXT_THEME = "history and contested explanation"
NEXT_GENRES = ["historiographical comparison", "source critique", "synthesis"]
...
def load_specs():
    ...
    if len(files) != 6:
        raise SystemExit(f"Expected 6 C2 Unit 7 spec files, found {len(files)}")
    return [load_json(path) for path in files]


def make_record(spec, idx):
    seq = START_SEQUENCE + idx
    pid = f"ur-c2-u07-p{idx + 1:02d}"
    ...
    return {
        ...
        "grammar_targets": [{
            "id": "ur-c2-u07-grammar-01",
            "role": "integration",
            "description": "Use conditional, concessive, contrastive, scope-sensitive, and comparative structures to distinguish literal meaning, contextual meaning, cultural cues, translation choices, and competing interpretations."
        }],
        "discourse_targets": [{
            "id": "ur-c2-u07-discourse-01",
            "role": "integration",
            "description": "Track cultural cues, semantic equivalence, translation choices, contextual meaning, interpretive horizons, reader effects, trade-offs, and evidence-based criticism across original synthetic translation scenarios."
        }],
        "questions": questions,
        "answer_key": answers,
        "speed_training": {
        ...
        "quality": {
            ...
            "notes": [
                ...
                "All literary lines, translations, translators, critics, workshops, and scenarios are original hypothetical instructional constructs; no sourced quotation or factual cultural claim is asserted."
            ]
        },
        "reader_tags": ["C2", "translation", "culture", "interpretation", "synthetic scenario"],
...
def validate_existing(existing, cont, status, plan):
    ...
    if [p.get("sequence") for p in existing] != list(range(1, EXPECTED_EXISTING + 1)):
        raise SystemExit("Existing Urdu C2 sequence is not exactly 1..36")
    if existing[-1].get("id") != "ur-c2-u06-p06":
    ...
    if "C2 Unit 7 / sequence 37" not in cont["active_frontier"]["production"]["action"]:
    ...
    if roadmap.get("theme") != THEME or roadmap.get("genres") != GENRES:
        raise SystemExit("ACTIVE_GENERATION_PLAN Unit 7 roadmap drift")
...
def validate_new_records(records):
    ...
    if [r["passage_type"] for r in records] != expected_types:
        raise SystemExit("Unit 7 passage-role cycle drift")
    if [r["sequence"] for r in records] != list(range(37, 43)):
        raise SystemExit("Unit 7 sequences must be 37..42")
    if len({r["id"] for r in records}) != 6:
        raise SystemExit("Duplicate Unit 7 passage ids")
    for r in records:
    ...
    if records[-1]["new_lexical_targets"]:
        raise SystemExit("C2 Unit 7 checkpoint must introduce no new lexical target")
...
def update_state(cont, status, plan):
    ...
    cont["active_frontier"]["production"]["action"] = "Continue generation-first production from Urdu C2 Unit 8 / sequence 43 using the canonical roadmap and ten-question contract."
    cont["exact_next_actions"] = [
        "Validate the routed state bundle and live canonical counts.",
        "Use reading/planning/ACTIVE_GENERATION_PLAN.json to start guarded Urdu C2 Unit 8 generation at sequence 43.",
...
def update_markdown():
    ...
    tasks = replace_required(tasks, "Canonical production frontier: **Urdu C2, Unit 7, sequence 37**.", "Canonical production frontier: **Urdu C2, Unit 8, sequence 43**.", "TASKS frontier")
    tasks = replace_required(tasks, "- Urdu: 336/360 generated; A1-C1 complete, C2 in progress.", "- Urdu: 342/360 generated; A1-C1 complete, C2 in progress.", "TASKS Urdu total")
    tasks = replace_required(tasks, "- Project: 1056/1080 generated.", "- Project: 1062/1080 generated.", "TASKS project total")
    TASKS.write_text(tasks, encoding="utf-8")

    handoff = HANDOFF.read_text(encoding="utf-8")
    handoff = replace_required(handoff, "- Canonical generated total: **1056**.", "- Canonical generated total: **1062**.", "handoff project total")
    handoff = replace_required(handoff, "- Urdu: **336/360**; A1-C1 generation complete and C2 generation in progress.", "- Urdu: **342/360**; A1-C1 generation complete and C2 generation in progress.", "handoff Urdu total")
    handoff = replace_required(handoff, "Continue **Urdu C2**, starting from Unit 7 / sequence 37, under:", "Continue **Urdu C2**, starting from Unit 8 / sequence 43, under:", "handoff frontier")
    handoff = replace_required(handoff, "C2 Unit 7 uses the roadmap theme **culture, translation, and interpretation** with `essay`, `comparative criticism`, and `reflection` genres.", "C2 Unit 8 uses the roadmap theme **history and contested explanation** with `historiographical comparison`, `source critique`, and `synthesis` genres.", "handoff roadmap")
    handoff = replace_required(handoff, "Run `python reading/tools/validate_continuation_state.py`; if it passes, resume guarded generation at **Urdu C2 Unit 7 / sequence 37** using the C2 Unit 7 roadmap theme `culture, translation, and interpretation`.", "Run `python reading/tools/validate_continuation_state.py`; if it passes, resume guarded generation at **Urdu C2 Unit 8 / sequence 43** using the C2 Unit 8 roadmap theme `history and contested explanation`.", "handoff exact next action")
...
def main():
    ...
    qa = {
        ...
        "release_claim": False,
        "next_frontier": "Urdu C2 Unit 8 / sequence 43",
        "notes": [
            "All six passages use original hypothetical literary, translation, and criticism scenarios; no sourced quotation or factual cultural claim is asserted.",
            "Five specialist Unit 7 lexical targets are marked beyond_base rather than assigned invented frequency ranks; the checkpoint introduces no new lexical target.",
