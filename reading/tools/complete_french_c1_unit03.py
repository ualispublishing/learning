...
def main():
 AUD.mkdir(parents=True,exist_ok=True);before=C1.read_bytes() if C1.exists() else None;before_exists=C1.exists();before_n=len(rows());generated=False;passed=False;error=None;stages=[]
 try:
  ...
  rs=rows()
  if len(rs)==12:
   run('generate_french_c1_unit03.py');generated=True;stages.append('generate_c1_unit03')
  ...
  run_env('sync_french_c1_unit_frontier.py',3);stages.append('sync_c1_unit03_to_unit04');passed=True
 except Exception:
  error=traceback.format_exc();print(error)
  if generated and not passed:
 ...
 result={'status':'PASS_TO_C1_UNIT04' if passed else 'C1_UNIT03_PENDING','date':'2026-08-17','starting_c1_passages':before_n,'ending_c1_passages':len(rows()),'b2_blob':h(B2),'c1_blob':h(C1),'c1_unit03_pass':passed,'completed_stages':stages,'error':error}
 if passed:
  lock=json.loads((AUD/'french_c1_unit03_frontier_lock.json').read_text(encoding='utf-8'));plan=json.loads((AUD/'french_c1_unit04_plan.json').read_text(encoding='utf-8'));probe=json.loads((AUD/'french_c1_unit04_target_probe.json').read_text(encoding='utf-8'));result.update({'accepted_c1_default':lock['accepted_c1_default_new_targets_per_standard_passage'],'default_is_hard_quota':False,'unit04_theme':plan.get('theme'),'unit04_genres':plan.get('genres'),'remaining_fresh_source_terms':probe.get('fresh_count')})
