...
STAGE = ROOT / "reading" / "audit" / "urdu_b2_u05_generation_candidate"
TARGET = ROOT / "reading" / "urdu" / "b2" / "passages.jsonl"
SCHEMA = ROOT / "reading" / "schema" / "passage.schema.json"
DATE = "2026-08-25"
EXPECTED_IDS = [f"ur-b2-u05-p{i:02d}" for i in range(1, 7)]
EXPECTED_SEQ = list(range(25, 31))
EXPECTED_TARGETS = [
    "ur-rank-2323", "ur-rank-2339",
    "ur-rank-2347", "ur-rank-2286",
    "ur-rank-2357", "ur-rank-2356",
    "ur-rank-2375", "ur-rank-2428",
    "ur-rank-2419", "ur-rank-2418",
]
EXPECTED_ROLES = ["instructional", "reinforcement", "interleaved", "transfer", "integration", "checkpoint"]
REQUIRED_GENRES = {"evidence summary", "news analysis", "argument"}
EXPECTED_ROADMAP = {
    "unit": 5,
...
def main() -> int:
    ...
    if len(b2) != 24 or [r.get("sequence") for r in b2] != list(range(1, 25)):
        fail("Urdu B2 must contain exactly sequences 1-24 before Unit 5 promotion")
    ...
    if status["current"]["canonical_passages"] != 924 or status["languages"]["urdu"]["canonical_passages"] != 204:
        fail("STATUS counts are no longer the reviewed B2 Unit 5 pre-promotion state")
    ...
    if plan.get("active_unit") != 5 or plan.get("start_sequence") != 25 or plan.get("existing_active_level_passages") != 24:
        fail("ACTIVE_GENERATION_PLAN no longer points to Urdu B2 Unit 5 / sequence 25")
    if plan.get("active_unit_roadmap") != EXPECTED_ROADMAP:
        fail("ACTIVE_GENERATION_PLAN Unit 5 roadmap drifted")
    action = continuation.get("active_frontier", {}).get("production", {}).get("action", "")
    if "Urdu B2 Unit 5 / sequence 25" not in action:
        fail("CONTINUATION frontier no longer points to Urdu B2 Unit 5 / sequence 25")
    ...
    for i in range(1, 7):
        path = STAGE / f"ur-b2-u05-p{i:02d}.json"
        if not path.exists():
            fail(f"missing candidate {path.relative_to(ROOT)}")
        row = json.loads(path.read_text(encoding="utf-8"))

        # Two reader-first wording repairs identified during pre-promotion review.
        if i == 1:
            row["answer_key"][9]["answer"] = row["answer_key"][9]["answer"].replace("ریکارڈ کی تسلسل", "ریکارڈ کے تسلسل")
        if i == 2:
            row["questions"][9]["prompt"] = "بہتر زرعی موسمی فیصلے میں کن شواہد کو یکجا کرنا چاہیے؟"
    ...
    if checkpoint_forms != expected_forms or any(exact_form_count(p6["text"], form) < 1 for form in expected_forms):
        fail("Unit 5 P6 does not visibly recycle all ten new targets")
    ...
    checks = {k: "PASS" for k in [
        ...
        "prior_b2_sequences_1_through_24_exact",
        "freshness_across_all_prior_urdu_target_ids",
        "record_count",
        "sequence_25_through_30",
        "role_cycle",
    ...
    (ROOT / "reading" / "audit" / "urdu_b2_u05_generation_validation_2026-08-25.json").write_text(json.dumps({
        "schema_version": 1,
        "project_id": "LANG-A1C2",
        "language": "ur",
        "cefr": "B2",
    ...
    (ROOT / "reading" / "audit" / "urdu_b2_u05_promotion_2026-08-25.json").write_text(json.dumps({
        "schema_version": 1,
        "project_id": "LANG-A1C2",
        "language": "ur",
        "cefr": "B2",
        ...
        "release_promotion": False,
        "before_record_count": 24,
        "after_record_count": 30,
    ...
    lexical_path = ROOT / "reading" / "audit" / "urdu_b2_u05_lexical_sense_check_2026-08-25.json"
    ...
    lexical["freshness_scope"] = "Exact target IDs checked against all canonical Urdu A1-B1 and B2 sequences 1-24 immediately before Unit 5 append."
    lexical_path.write_text(json.dumps(lexical, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")

    status["current"]["canonical_passages"] = 930
    status["current"]["remaining_generation_passages"] = 150
    urdu_status = status["languages"]["urdu"]
    urdu_status["generation_state"] = "B2_IN_PROGRESS"
    urdu_status["canonical_passages"] = 210
    urdu_status["remaining_generation_passages"] = 150
    urdu_status["next_generation_level"] = "B2"
    status_path.write_text(json.dumps(status, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")

    continuation["updated"] = DATE
    continuation["production"]["canonical_passages"] = 930
    continuation["production"]["urdu"]["canonical_passages"] = 210
    continuation["active_frontier"]["production"]["action"] = "Continue generation-first production from Urdu B2 Unit 6 / sequence 31 using the canonical roadmap and ten-question contract."
    if len(continuation.get("exact_next_actions", [])) < 2:
        fail("CONTINUATION exact_next_actions is unexpectedly short")
    continuation["exact_next_actions"][1] = "Use reading/planning/ACTIVE_GENERATION_PLAN.json to start guarded Urdu B2 Unit 6 generation at sequence 31."
    ...
    tasks = replace_once(tasks, "Canonical production frontier: **Urdu B2, Unit 5, sequence 25**.", "Canonical production frontier: **Urdu B2, Unit 6, sequence 31**.", "TASKS frontier")
    tasks = replace_once(tasks, "- Urdu: 204/360 generated; A1-B1 complete, B2 in progress.", "- Urdu: 210/360 generated; A1-B1 complete, B2 in progress.", "TASKS Urdu total")
    tasks = replace_once(tasks, "- Project: 924/1080 generated.", "- Project: 930/1080 generated.", "TASKS project total")
    tasks_path.write_text(tasks, encoding="utf-8")

    handoff = handoff_path.read_text(encoding="utf-8")
    handoff = replace_once(handoff, "- Canonical generated total: **924**.", "- Canonical generated total: **930**.", "HANDOFF project total")
    handoff = replace_once(handoff, "- Urdu: **204/360**; A1-B1 generation complete and B2 generation in progress.", "- Urdu: **210/360**; A1-B1 generation complete and B2 generation in progress.", "HANDOFF Urdu total")
    handoff = replace_once(handoff, "Continue **Urdu B2**, starting from Unit 5 / sequence 25, under:", "Continue **Urdu B2**, starting from Unit 6 / sequence 31, under:", "HANDOFF frontier")
    handoff = replace_once(handoff, "B2 Unit 5 uses the roadmap theme **climate and uncertainty** with `evidence summary`, `news analysis`, and `argument` genres.", "B2 Unit 6 uses the roadmap theme **digital life and privacy** with `analysis`, `policy-style summary`, and `paired opinions` genres.", "HANDOFF roadmap")
    handoff = replace_once(handoff, "Run `python reading/tools/validate_continuation_state.py`; if it passes, resume guarded generation at **Urdu B2 Unit 5 / sequence 25** using the B2 Unit 5 roadmap theme `climate and uncertainty`.", "Run `python reading/tools/validate_continuation_state.py`; if it passes, resume guarded generation at **Urdu B2 Unit 6 / sequence 31** using the B2 Unit 6 roadmap theme `digital life and privacy`.", "HANDOFF next action")
    handoff_path.write_text(handoff, encoding="utf-8")

    print("Urdu B2 Unit 5 promotion validation: PASS")
    print("canonical B2 passages: 30; project passages: 930; Urdu passages: 210")
    print("next frontier: Urdu B2 Unit 6 / sequence 31 — digital life and privacy")
    return 0
