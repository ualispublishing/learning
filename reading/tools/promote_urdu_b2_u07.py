...
STAGE = ROOT / "reading" / "audit" / "urdu_b2_u07_generation_candidate"
...
DATE = "2026-08-26"
EXPECTED_IDS = [f"ur-b2-u07-p{i:02d}" for i in range(1, 7)]
EXPECTED_SEQ = list(range(37, 43))
EXPECTED_TARGETS = [
    "ur-rank-2514", "ur-rank-2534",
    "ur-rank-2532", "ur-rank-2583",
    "ur-rank-2536", "ur-rank-2546",
    "ur-rank-2579", "ur-rank-2544",
    "ur-rank-2586", "ur-rank-2587",
]
EXPECTED_ROLES = ["instructional", "reinforcement", "interleaved", "transfer", "integration", "checkpoint"]
REQUIRED_GENRES = {"review", "profile", "critical comparison"}
EXPECTED_ROADMAP = {
    "unit": 7,
...
def main() -> int:
    ...
    if len(b2) != 36 or [r.get("sequence") for r in b2] != list(range(1, 37)):
        fail("Urdu B2 must contain exactly sequences 1-36 before Unit 7 promotion")
    ...
    if status["current"]["canonical_passages"] != 936 or status["languages"]["urdu"]["canonical_passages"] != 216:
        fail("STATUS counts are no longer the reviewed B2 Unit 7 pre-promotion state")
    if status["current"]["active_language"] != "urdu" or status["current"]["active_level"] != "B2":
        fail("STATUS active frontier is no longer Urdu B2")
    if plan.get("active_unit") != 7 or plan.get("start_sequence") != 37 or plan.get("existing_active_level_passages") != 36:
        fail("ACTIVE_GENERATION_PLAN no longer points to Urdu B2 Unit 7 / sequence 37")
    if plan.get("active_unit_roadmap") != EXPECTED_ROADMAP:
        fail("ACTIVE_GENERATION_PLAN Unit 7 roadmap drifted")
    action = continuation.get("active_frontier", {}).get("production", {}).get("action", "")
    if "Urdu B2 Unit 7 / sequence 37" not in action:
        fail("CONTINUATION frontier no longer points to Urdu B2 Unit 7 / sequence 37")
    ...
    if collision:
        fail(f"Unit 7 target-ID freshness collision: {collision}")

    canonical_quality = json.loads(json.dumps(b2[-1]["quality"]))
    rows: list[dict] = []
    learner_parts: list[str] = []
    for i in range(1, 7):
        path = STAGE / f"ur-b2-u07-p{i:02d}.json"
        if not path.exists():
    ...
    final = jl(TARGET)
    if len(final) != 42 or [r["sequence"] for r in final] != list(range(1, 43)) or [r["id"] for r in final[-6:]] != EXPECTED_IDS:
        fail("canonical B2 post-append check failed")

    STAGE.mkdir(parents=True, exist_ok=True)
    (STAGE / "manifest.json").write_text(json.dumps({
        "schema_version": 1, "project_id": "LANG-A1C2", "language": "ur", "cefr": "B2", "unit": 7,
        "date": DATE, "status": "CANONICALIZED", "canonical_target": "reading/urdu/b2/passages.jsonl",
        "sequence_range": [37, 42], "record_count": 6, "release_promotion": False,
    }, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")

    checks = {k: "PASS" for k in [
        "prior_a1_a2_b1_60_each", "prior_b2_sequences_1_through_36_exact",
        "freshness_across_all_prior_urdu_target_ids", "record_count", "sequence_37_through_42", "role_cycle",
        "question_answer_10x10", "bidirectional_links", "new_target_distribution_2_2_2_2_2_0",
        "new_target_text_exposure", "first_introduction_order_unicode_word_boundary", "required_genres",
        "p6_checkpoint_policy", "p6_all_target_recycling", "learner_script_scan",
        "schema_required_fields_enums_context_and_review_metadata", "review_target_identity_check",
        "b2_word_band_350_550", "quality_metadata_preserved_from_prior_canonical_b2",
        "preexisting_canonical_bytes_preserved", "release_status_unchanged",
    ]}
    (ROOT / "reading" / "audit" / "urdu_b2_u07_generation_validation_2026-08-26.json").write_text(json.dumps({
        "schema_version": 1, "project_id": "LANG-A1C2", "language": "ur", "cefr": "B2", "unit": 7,
        "date": DATE, "canonicalized": True, "release_promotion": False,
        "word_counts": [r["word_count"] for r in rows], "checks": checks,
    }, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    (ROOT / "reading" / "audit" / "urdu_b2_u07_promotion_2026-08-26.json").write_text(json.dumps({
        "schema_version": 1, "project_id": "LANG-A1C2", "language": "ur", "cefr": "B2", "unit": 7,
        "date": DATE, "status": "CANONICAL_PROMOTION_PASS", "release_promotion": False,
        "before_record_count": 36, "after_record_count": 42, "appended_sequences": EXPECTED_SEQ,
        "appended_ids": EXPECTED_IDS, "preexisting_bytes_preserved_exactly": True,
        "canonical_sha256_before": before_sha, "canonical_sha256_after": hashlib.sha256(after).hexdigest(),
    }, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")

    lexical_path = ROOT / "reading" / "audit" / "urdu_b2_u07_lexical_sense_check_2026-08-26.json"
    ...
    lexical["freshness_scope"] = "Exact target IDs checked against all canonical Urdu A1-B1 and B2 sequences 1-36 immediately before Unit 7 append."
    lexical_path.write_text(json.dumps(lexical, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")

    status["current"]["canonical_passages"] = 942
    status["current"]["remaining_generation_passages"] = 138
    urdu_status = status["languages"]["urdu"]
    urdu_status["generation_state"] = "B2_IN_PROGRESS"
    urdu_status["canonical_passages"] = 222
    urdu_status["remaining_generation_passages"] = 138
    urdu_status["next_generation_level"] = "B2"
    status_path.write_text(json.dumps(status, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")

    continuation["updated"] = DATE
    continuation["production"]["canonical_passages"] = 942
    continuation["production"]["urdu"]["canonical_passages"] = 222
    continuation["active_frontier"]["production"]["action"] = "Continue generation-first production from Urdu B2 Unit 8 / sequence 43 using the canonical roadmap and ten-question contract."
    if len(continuation.get("exact_next_actions", [])) < 2:
        fail("CONTINUATION exact_next_actions is unexpectedly short")
    continuation["exact_next_actions"][1] = "Use reading/planning/ACTIVE_GENERATION_PLAN.json to start guarded Urdu B2 Unit 8 generation at sequence 43."
    ...
    tasks = replace_once(tasks, "Updated: 2026-08-25", f"Updated: {DATE}", "TASKS date")
    tasks = replace_once(tasks, "Canonical production frontier: **Urdu B2, Unit 7, sequence 37**.", "Canonical production frontier: **Urdu B2, Unit 8, sequence 43**.", "TASKS frontier")
    tasks = replace_once(tasks, "- Urdu: 216/360 generated; A1-B1 complete, B2 in progress.", "- Urdu: 222/360 generated; A1-B1 complete, B2 in progress.", "TASKS Urdu total")
    tasks = replace_once(tasks, "- Project: 936/1080 generated.", "- Project: 942/1080 generated.", "TASKS project total")
    tasks_path.write_text(tasks, encoding="utf-8")

    handoff = handoff_path.read_text(encoding="utf-8")
    handoff = replace_once(handoff, "Updated: 2026-08-25", f"Updated: {DATE}", "HANDOFF date")
    handoff = replace_once(handoff, "- Canonical generated total: **936**.", "- Canonical generated total: **942**.", "HANDOFF project total")
    handoff = replace_once(handoff, "- Urdu: **216/360**; A1-B1 generation complete and B2 generation in progress.", "- Urdu: **222/360**; A1-B1 generation complete and B2 generation in progress.", "HANDOFF Urdu total")
    handoff = replace_once(handoff, "Continue **Urdu B2**, starting from Unit 7 / sequence 37, under:", "Continue **Urdu B2**, starting from Unit 8 / sequence 43, under:", "HANDOFF frontier")
    handoff = replace_once(handoff, "B2 Unit 7 uses the roadmap theme **arts and interpretation** with `review`, `profile`, and `critical comparison` genres.", "B2 Unit 8 uses the roadmap theme **history and explanation** with `historical account`, `causal analysis`, and `source comparison` genres.", "HANDOFF roadmap")
    handoff = replace_once(handoff, "Run `python reading/tools/validate_continuation_state.py`; if it passes, resume guarded generation at **Urdu B2 Unit 7 / sequence 37** using the B2 Unit 7 roadmap theme `arts and interpretation`.", "Run `python reading/tools/validate_continuation_state.py`; if it passes, resume guarded generation at **Urdu B2 Unit 8 / sequence 43** using the B2 Unit 8 roadmap theme `history and explanation`.", "HANDOFF next action")
    handoff_path.write_text(handoff, encoding="utf-8")

    if RELEASE.read_bytes() != release_before:
        fail("RELEASE_STATUS.json changed during generation promotion")

    print("Urdu B2 Unit 7 promotion validation: PASS")
    print("canonical B2 passages: 42; project passages: 942; Urdu passages: 222")
    print("next frontier: Urdu B2 Unit 8 / sequence 43 — history and explanation")
    return 0
