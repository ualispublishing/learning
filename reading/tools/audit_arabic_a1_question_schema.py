...
results=[]; problems=[]
for r in rows:
    pid=r['id']; qs=r.get('questions',[]); ans=r.get('answer_key',[])
    ...
    results.append({'id':pid,'questions':len(qs),'answers':len(ans),'gate':'PASS' if not local else 'FAIL','problems':local})

summary={'passage_count':len(rows),'expected_passages':6,'expected_questions_per_passage':10,'expected_total_questions':60,'actual_total_questions':sum(len(r.get('questions',[])) for r in rows),'actual_total_answers':sum(len(r.get('answer_key',[])) for r in rows),'passages_passing':sum(x['gate']=='PASS' for x in results),'problems':problems,'gate':'PASS' if len(rows)==6 and not problems else 'FAIL','passages':results}
OUT.write_text(json.dumps(summary,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
print(json.dumps(summary,ensure_ascii=False,indent=2))
if summary['gate']!='PASS': raise SystemExit('Arabic A1 question schema failed')
