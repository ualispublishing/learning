...
QA = ROOT / "reading/audit/urdu_c1_u08_generation_qa_2026-08-28.json"
SPEC_DIR = ROOT / "reading/tools/unit08_specs"

UNIT = 8
START_SEQUENCE = 43
EXPECTED_EXISTING = 42
EXPECTED_PROJECT = 1002
EXPECTED_URDU = 282
NEW_PROJECT = 1008
NEW_URDU = 288
NEW_REMAINING = 72
...
NEXT_THEME = "history, memory, and narrative"
NEXT_GENRES = ["historiographical essay", "source comparison", "reflection"]
...
def decode_specs():
    specs = [load_json(SPEC_DIR / f"p{n:02d}.json") for n in range(1, 7)]
    if len(specs) != 6:
        raise SystemExit(f"Expected 6 Unit 8 specs, found {len(specs)}")
    return specs
...
def make_record(spec, idx):
    ...
    questions = []
    answers = []
    qrows = spec["questions"]
    arows = spec["answers"]
    if len(qrows) != 10 or len(arows) != 10:
        raise SystemExit(f"{pid} must have exactly 10 questions and 10 answers")
    for n, ((qtype, prompt), answer) in enumerate(zip(qrows, arows), 1):
    ...
    return {
        ...
        "grammar_targets": [{
            "id": "ur-c1-u08-grammar-01",
            "role": "integration",
            "description": "Use conditional, modal, and evidential phrasing to distinguish assumptions, possible outcomes, limits, and recommendations."
        }],
        "discourse_targets": [{
            "id": "ur-c1-u08-discourse-01",
            "role": "integration",
            "description": "Separate assumptions, uncertainty, scenarios, consequences, decision thresholds, and update rules across an analytical argument."
        }],
        "questions": questions,
        "answer_key": answers,
        "speed_training": {
        ...
        "quality": {
            ...
            "notes": [
                ...
                "The institution, organization, company, program, figures, and scenarios are explicitly hypothetical instructional constructs rather than factual claims."
            ]
        },
        "reader_tags": ["C1", "economics", "risk", "forecasting", "synthetic scenario"],
...
def validate_existing(existing, cont, status, plan):
    ...
    seqs = [p.get("sequence") for p in existing]
    if seqs != list(range(1, EXPECTED_EXISTING + 1)):
        raise SystemExit("Existing Urdu C1 sequence is not exactly 1..42")
    if existing[-1].get("id") != "ur-c1-u07-p06":
    ...
    if "Unit 8 / sequence 43" not in action:
        raise SystemExit(f"Unexpected continuation frontier: {action}")
    ...
    if roadmap.get("theme") != THEME or roadmap.get("genres") != ["analysis", "briefing", "scenario comparison"]:
...
def validate_new_records(records):
    ...
    valid_types = {"instructional", "reinforcement", "transfer", "interleaved", "fluency", "stretch", "checkpoint", "paired", "integration"}
    for r in records:
        if r["passage_type"] not in valid_types:
            raise SystemExit(f"Invalid passage_type in {r['id']}")
...
def update_state(cont, status, plan):
    ...
    cont["active_frontier"]["production"]["action"] = (
        "Continue generation-first production from Urdu C1 Unit 9 / sequence 49 using the canonical roadmap and ten-question contract."
    )
    cont["exact_next_actions"] = [
        "Validate the routed state bundle and live canonical counts.",
        "Use reading/planning/ACTIVE_GENERATION_PLAN.json to start guarded Urdu C1 Unit 9 generation at sequence 49.",
...
def update_markdown():
    ...
    tasks = replace_required(tasks,
        "Canonical production frontier: **Urdu C1, Unit 8, sequence 43**.",
        "Canonical production frontier: **Urdu C1, Unit 9, sequence 49**.",
        "TASKS frontier")
    tasks = replace_required(tasks,
        "- Urdu: 282/360 generated; A1-B2 complete, C1 in progress.",
        "- Urdu: 288/360 generated; A1-B2 complete, C1 in progress.",
        "TASKS Urdu total")
    tasks = replace_required(tasks,
        "- Project: 1002/1080 generated.",
...
def main():
    ...
    records = [make_record(spec, idx) for idx, spec in enumerate(decode_specs())]
    ...
    qa = {
        ...
        "release_claim": False,
        "next_frontier": "Urdu C1 Unit 9 / sequence 49",
        "notes": [
            "All six passages use explicitly hypothetical instructional scenarios; no factual institution or market claim is asserted.",
            ...
            "Generation-stage QA preserves the ten-question/ten-answer contract and keeps educator/publication release review separate."
        ]
    }
    dump_json(QA, qa)
