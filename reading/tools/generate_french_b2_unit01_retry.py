...
Repairs exact `général` in P04/P06, B1 bridge reviews `dangereux`/`risquer`, and
one stale cross-passage assessment tag. The 20-word calibration pool and all
...
if src.count(old)!=1: raise AssertionError(f'expected one P04 general-exposure location, found {src.count(old)}')
...
old_cp="les participants visibles sont pris pour une image générale de la population."
new_cp="les participants visibles sont pris pour un portrait général de la population."
if src.count(old_cp)!=1: raise AssertionError(f'expected one P06 general-exposure location, found {src.count(old_cp)}')
