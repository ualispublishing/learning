...
STAGE = ROOT / "reading" / "audit" / "urdu_b2_u08_generation_candidate"
...
DATE = "2026-08-26"
EXPECTED_IDS = [f"ur-b2-u08-p{i:02d}" for i in range(1, 7)]
EXPECTED_SEQ = list(range(43, 49))
EXPECTED_TARGETS = [
    "ur-rank-2612", "ur-rank-2647",
    "ur-rank-2631", "ur-rank-2627",
    "ur-rank-2608", "ur-rank-2658",
    "ur-rank-2656", "ur-rank-2650",
    "ur-rank-2620", "ur-rank-2623",
]
EXPECTED_ROLES = ["instructional", "reinforcement", "interleaved", "transfer", "integration", "checkpoint"]
REQUIRED_GENRES = {"historical account", "causal analysis", "source comparison"}
EXPECTED_ROADMAP = {
    "unit": 8,
    "theme": "history and explanation",
    "genres": ["historical account", "causal analysis", "source comparison"],
}
NEXT_ROADMAP = {
    "unit": 9,
    "theme": "public policy and trade-offs",
    "genres": ["briefing", "argument", "counterargument"],
}
...
def main() -> int:
    ...
    if len(b2) != 42 or [r.get("sequence") for r in b2] != list(range(1, 43)):
        fail("Urdu B2 must contain exactly sequences 1-42 before Unit 8 promotion")
    ...
    if status["current"]["canonical_passages"] != 942 or status["languages"]["urdu"]["canonical_passages"] != 222:
        fail("STATUS counts are no longer the reviewed B2 Unit 8 pre-promotion state")
    if status["current"]["active_language"] != "urdu" or status["current"]["active_level"] != "B2":
        fail("STATUS active frontier is no longer Urdu B2")
    if plan.get("active_unit") != 8 or plan.get("start_sequence") != 43 or plan.get("existing_active_level_passages") != 42:
        fail("ACTIVE_GENERATION_PLAN no longer points to Urdu B2 Unit 8 / sequence 43")
    if plan.get("active_unit_roadmap") != EXPECTED_ROADMAP:
        fail("ACTIVE_GENERATION_PLAN Unit 8 roadmap drifted")
    action = continuation.get("active_frontier", {}).get("production", {}).get("action", "")
    if "Urdu B2 Unit 8 / sequence 43" not in action:
        fail("CONTINUATION frontier no longer points to Urdu B2 Unit 8 / sequence 43")
    ...
    if collision:
        fail(f"Unit 8 target-ID freshness collision: {collision}")

    canonical_quality = json.loads(json.dumps(b2[-1]["quality"]))
    rows: list[dict] = []
    learner_parts: list[str] = []
    for i in range(1, 7):
        path = STAGE / f"ur-b2-u08-p{i:02d}.json"
        if not path.exists():
            fail(f"missing candidate {path.relative_to(ROOT)}")
        row = json.loads(path.read_text(encoding="utf-8"))

        # Reader-first repairs from the pre-promotion Urdu naturalness pass.
        if i == 1:
            row["text"] = row["text"].replace("قصبے کی جغرافیہ ایسی تھی", "قصبے کا جغرافیہ ایسا تھا")
            row["text"] = row["text"].replace("جغرافیہ یہ بتا سکتی ہے", "جغرافیہ یہ بتا سکتا ہے")
        elif i == 2:
            row["text"] = row["text"].replace("تیار کی گئی سببی تجزیے میں", "تیار کیے گئے سببی تجزیے میں")
            row["text"] = row["text"].replace(
                "مضبوط سببی تجزیے نہ تو ہر واقعے کو برابر اہم سمجھتی ہے اور نہ کسی ایک واقعے کو پورا جواب بناتی ہے؛ وہ ترتیب، متبادل وجہ، مقصد اور طویل مدتی نتیجے کو ایک ساتھ جانچتی ہے۔",
                "مضبوط سببی تجزیہ نہ تو ہر واقعے کو برابر اہم سمجھتا ہے اور نہ کسی ایک واقعے کو پورا جواب بناتا ہے؛ وہ ترتیب، متبادل وجہ، مقصد اور طویل مدتی نتیجے کو ایک ساتھ جانچتا ہے۔",
            )
        elif i == 3:
            row["text"] = row["text"].replace("تیسرے قسم کے شواہد", "تیسری قسم کے شواہد")
        elif i == 4:
            row["text"] = row["text"].replace("اچھی تاریخی بیان", "اچھا تاریخی بیان")
        elif i == 5:
            row["text"] = row["text"].replace("بہتر سببی تجزیہ یہ پوچھتی ہے", "بہتر سببی تجزیہ یہ پوچھتا ہے")
    ...
    for row in rows:
        ...
        for rt in row.get("review_lexical_targets", []):
            if rt["representation"] not in review_reps or rt["review_stage"] not in review_stages:
                fail(f"invalid review metadata: {row['id']}/{rt['id']}")
    ...
    final = jl(TARGET)
    if len(final) != 48 or [r["sequence"] for r in final] != list(range(1, 49)) or [r["id"] for r in final[-6:]] != EXPECTED_IDS:
    ...
    checks = {k: "PASS" for k in [
        ...
        "prior_b2_sequences_1_through_42_exact",
        "freshness_across_all_prior_urdu_target_ids",
        "record_count",
        "sequence_43_through_48",
        "role_cycle",
        ...
        "reader_first_wording_repairs",
        "release_status_unchanged",
    ]}
    (ROOT / "reading" / "audit" / "urdu_b2_u08_generation_validation_2026-08-26.json").write_text(json.dumps({
        "schema_version": 1,
        "project_id": "LANG-A1C2",
        "language": "ur",
        "cefr": "B2",
    ...
    (ROOT / "reading" / "audit" / "urdu_b2_u08_promotion_2026-08-26.json").write_text(json.dumps({
        "schema_version": 1,
        "project_id": "LANG-A1C2",
        "language": "ur",
        "cefr": "B2",
    ...
    lexical_path = ROOT / "reading" / "audit" / "urdu_b2_u08_lexical_sense_check_2026-08-26.json"
    ...
    lexical["freshness_scope"] = "Exact target IDs checked against all canonical Urdu A1-B1 and B2 sequences 1-42 immediately before Unit 8 append."
    lexical_path.write_text(json.dumps(lexical, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")

    status["current"]["canonical_passages"] = 948
    status["current"]["remaining_generation_passages"] = 132
    urdu_status = status["languages"]["urdu"]
    urdu_status["generation_state"] = "B2_IN_PROGRESS"
    urdu_status["canonical_passages"] = 228
    urdu_status["remaining_generation_passages"] = 132
    urdu_status["next_generation_level"] = "B2"
    status_path.write_text(json.dumps(status, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")

    continuation["updated"] = DATE
    continuation["production"]["canonical_passages"] = 948
    continuation["production"]["urdu"]["canonical_passages"] = 228
    continuation["active_frontier"]["production"]["action"] = "Continue generation-first production from Urdu B2 Unit 9 / sequence 49 using the canonical roadmap and ten-question contract."
    if len(continuation.get("exact_next_actions", [])) < 2:
        fail("CONTINUATION exact_next_actions is unexpectedly short")
    continuation["exact_next_actions"][1] = "Use reading/planning/ACTIVE_GENERATION_PLAN.json to start guarded Urdu B2 Unit 9 generation at sequence 49."
    ...
    tasks = replace_once(tasks, "Canonical production frontier: **Urdu B2, Unit 8, sequence 43**.", "Canonical production frontier: **Urdu B2, Unit 9, sequence 49**.", "TASKS frontier")
    tasks = replace_once(tasks, "- Urdu: 222/360 generated; A1-B1 complete, B2 in progress.", "- Urdu: 228/360 generated; A1-B1 complete, B2 in progress.", "TASKS Urdu total")
    tasks = replace_once(tasks, "- Project: 942/1080 generated.", "- Project: 948/1080 generated.", "TASKS project total")
    tasks_path.write_text(tasks, encoding="utf-8")

    handoff = handoff_path.read_text(encoding="utf-8")
    handoff = replace_once(handoff, "- Canonical generated total: **942**.", "- Canonical generated total: **948**.", "HANDOFF project total")
    handoff = replace_once(handoff, "- Urdu: **222/360**; A1-B1 generation complete and B2 generation in progress.", "- Urdu: **228/360**; A1-B1 generation complete and B2 generation in progress.", "HANDOFF Urdu total")
    handoff = replace_once(handoff, "Continue **Urdu B2**, starting from Unit 8 / sequence 43, under:", "Continue **Urdu B2**, starting from Unit 9 / sequence 49, under:", "HANDOFF frontier")
    handoff = replace_once(handoff, "B2 Unit 8 uses the roadmap theme **history and explanation** with `historical account`, `causal analysis`, and `source comparison` genres.", "B2 Unit 9 uses the roadmap theme **public policy and trade-offs** with `briefing`, `argument`, and `counterargument` genres.", "HANDOFF roadmap")
    handoff = replace_once(handoff, "Run `python reading/tools/validate_continuation_state.py`; if it passes, resume guarded generation at **Urdu B2 Unit 8 / sequence 43** using the B2 Unit 8 roadmap theme `history and explanation`.", "Run `python reading/tools/validate_continuation_state.py`; if it passes, resume guarded generation at **Urdu B2 Unit 9 / sequence 49** using the B2 Unit 9 roadmap theme `public policy and trade-offs`.", "HANDOFF next action")
    handoff_path.write_text(handoff, encoding="utf-8")

    if RELEASE.read_bytes() != release_before:
        fail("RELEASE_STATUS.json changed during a generation-only promotion")

    print("Urdu B2 Unit 8 promotion validation: PASS")
    print("canonical B2 passages: 48; project passages: 948; Urdu passages: 228")
    print("next frontier: Urdu B2 Unit 9 / sequence 49 — public policy and trade-offs")
    return 0
