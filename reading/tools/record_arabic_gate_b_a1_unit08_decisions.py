#!/usr/bin/env python3
"""Record fresh Arabic Gate B A1 Unit 8 decisions from the rebuilt Gate B inventory."""
from __future__ import annotations
import hashlib,importlib.util,json
from pathlib import Path
ROOT=Path(__file__).resolve().parents[2]; READING=ROOT/'reading'; CANONICAL=READING/'arabic/a1/passages.jsonl'; INVENTORY=READING/'audit/arabic_gate_b_naturalness_inventory_2026-08-30.json'; DECISION_DIR=READING/'audit/arabic_gate_b_decisions_2026-08-30'; DECISION_PATH=DECISION_DIR/'a1_u08.json'; EXPECTED_IDS=[f'ar-a1-u08-p{i:02d}' for i in range(1,7)]; EXPECTED_FINDINGS=12
def sha(data:bytes)->str:return hashlib.sha256(data).hexdigest()
def load_meta():
 source=Path(__file__).with_name('apply_arabic_gate_b_a1_unit08.py'); spec=importlib.util.spec_from_file_location('arabic_gate_b_a1_u08_meta',source)
 if spec is None or spec.loader is None: raise SystemExit('cannot load Unit 8 repair metadata')
...
def main():
 ...
 if set(meta)!=set(EXPECTED_IDS) or sum(len(meta[p]) for p in EXPECTED_IDS)!=EXPECTED_FINDINGS: raise SystemExit('Unit 8 finding metadata drift')
 decisions=[]
 ...
 doc={'schema_version':1,'project_id':'LANG-A1C2','language':'arabic','level':'A1','unit':8,'date':'2026-08-31','gate':'Gate B — passage-by-passage linguistic/naturalness audit','canonical_path':'reading/arabic/a1/passages.jsonl','canonical_sha256':canonical_sha,'records_reviewed':6,'records_with_findings':6,'fresh_findings':EXPECTED_FINDINGS,'decisions':decisions,'quality_promotion':False,'release_claim':False,'guard':'Learner-facing hashes come only from the freshly rebuilt authoritative Gate B inventory and are independently revalidated by the progress synchronizer.'}
 DECISION_DIR.mkdir(parents=True,exist_ok=True); DECISION_PATH.write_text(json.dumps(doc,ensure_ascii=False,indent=2)+'\n',encoding='utf-8'); print(json.dumps({'level':'A1','unit':8,'records_reviewed':6,'fresh_findings':EXPECTED_FINDINGS,'canonical_sha256':canonical_sha},ensure_ascii=False,indent=2))
