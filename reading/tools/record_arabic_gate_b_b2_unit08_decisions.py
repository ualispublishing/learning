#!/usr/bin/env python3
"""Record fresh Arabic Gate B B2 Unit 8 decisions."""
...
DECISION=READING/'audit/arabic_gate_b_decisions_2026-08-30/b2_u08.json'
IDS=[f'ar-b2-u08-p{i:02d}' for i in range(1,7)]; FINDINGS=3; WITH=3
def sha(b): return hashlib.sha256(b).hexdigest()
def meta():
 p=Path(__file__).with_name('apply_arabic_gate_b_b2_unit08.py'); s=importlib.util.spec_from_file_location('u08',p); m=importlib.util.module_from_spec(s); s.loader.exec_module(m); return m.FINDING_META
def main():
 raw=CANONICAL.read_bytes(); rows=[json.loads(x) for x in raw.decode().splitlines() if x.strip()]; csha=sha(raw)
 if len(rows)!=60 or [rows[i].get('id') for i in range(42,48)]!=IDS: raise SystemExit('B2 Unit 8 layout/id drift')
 for r in rows[42:48]:
  q=r.get('quality',{})
 ...
 doc={'schema_version':1,'project_id':'LANG-A1C2','language':'arabic','level':'B2','unit':8,'date':'2026-09-04','gate':'Gate B — passage-by-passage linguistic/naturalness audit','canonical_path':'reading/arabic/b2/passages.jsonl','canonical_sha256':csha,'records_reviewed':6,'records_with_findings':WITH,'fresh_findings':FINDINGS,'decisions':decisions,'quality_promotion':False,'release_claim':False,'guard':'Learner-facing hashes come only from the freshly rebuilt authoritative Gate B inventory and are independently revalidated by the progress synchronizer. Legitimate B2 grammar-in-context/discourse analysis is retained; this internal Gate B pass does not constitute educator/publication release approval.'}
