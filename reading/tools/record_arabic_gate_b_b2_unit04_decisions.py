#!/usr/bin/env python3
"""Record fresh Arabic Gate B B2 Unit 4 decisions from the rebuilt Gate B inventory."""
...
DECISION_PATH = DECISION_DIR / "b2_u04.json"
EXPECTED_IDS = [f"ar-b2-u04-p{i:02d}" for i in range(1, 7)]
EXPECTED_FINDINGS = 6
EXPECTED_WITH_FINDINGS = 4
...
def load_meta():
    source = Path(__file__).with_name("apply_arabic_gate_b_b2_unit04.py")
    spec = importlib.util.spec_from_file_location("arabic_gate_b_b2_u04_meta", source)
    if spec is None or spec.loader is None:
        raise SystemExit("cannot load B2 Unit 4 repair metadata")
...
def main() -> None:
    ...
    if len(rows) != 60 or [row.get("sequence") for row in rows] != list(range(1, 61)) or [rows[i].get("id") for i in range(18, 24)] != EXPECTED_IDS:
        raise SystemExit("B2 Unit 4 layout/id drift")

    for record in rows[18:24]:
    ...
    if set(meta) != set(EXPECTED_IDS) or sum(len(meta[pid]) for pid in EXPECTED_IDS) != EXPECTED_FINDINGS or sum(bool(meta[pid]) for pid in EXPECTED_IDS) != EXPECTED_WITH_FINDINGS:
        raise SystemExit("B2 Unit 4 finding metadata drift")
