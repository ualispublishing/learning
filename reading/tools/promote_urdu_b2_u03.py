...
STAGE = ROOT / "reading" / "audit" / "urdu_b2_u03_generation_candidate"
TARGET = ROOT / "reading" / "urdu" / "b2" / "passages.jsonl"
SCHEMA = ROOT / "reading" / "schema" / "passage.schema.json"
DATE = "2026-08-24"

EXPECTED_IDS = [f"ur-b2-u03-p{i:02d}" for i in range(1, 7)]
EXPECTED_SEQ = list(range(13, 19))
EXPECTED_TARGETS = [
    "ur-rank-2005", "ur-rank-2004",
    "ur-rank-2118", "ur-rank-2350",
    "ur-rank-2378", "ur-rank-2352",
    "ur-rank-2320", "ur-rank-2056",
    "ur-rank-2062", "ur-rank-2025",
]
EXPECTED_ROLES = ["instructional", "reinforcement", "interleaved", "transfer", "integration", "checkpoint"]
REQUIRED_GENRES = {"argument", "case", "response"}
...
def main() -> int:
    ...
    if len(b2) != 12 or [r.get("sequence") for r in b2] != list(range(1, 13)):
        fail("Urdu B2 must contain exactly sequences 1-12 before Unit 3 promotion")
    ...
    if status["current"]["canonical_passages"] != 912 or status["languages"]["urdu"]["canonical_passages"] != 192:
        fail("STATUS counts are no longer the reviewed B2 Unit 3 pre-promotion state")
    if status["current"]["active_language"] != "urdu" or status["current"]["active_level"] != "B2":
        fail("STATUS active frontier is no longer Urdu B2")
    if plan.get("active_unit") != 3 or plan.get("start_sequence") != 13 or plan.get("existing_active_level_passages") != 12:
        fail("ACTIVE_GENERATION_PLAN no longer points to Urdu B2 Unit 3 / sequence 13")
    action = continuation.get("active_frontier", {}).get("production", {}).get("action", "")
    if "Urdu B2 Unit 3 / sequence 13" not in action:
        fail("CONTINUATION frontier no longer points to Urdu B2 Unit 3 / sequence 13")
    ...
    if collision:
        fail(f"Unit 3 target-ID freshness collision: {collision}")

    rows: list[dict] = []
    learner_parts: list[str] = []
    for i in range(1, 7):
        path = STAGE / f"ur-b2-u03-p{i:02d}.json"
        if not path.exists():
    ...
    (STAGE / "manifest.json").write_text(json.dumps({
        "schema_version": 1, "project_id": "LANG-A1C2", "language": "ur", "cefr": "B2", "unit": 3,
    ...
    checks = {k: "PASS" for k in [
        "prior_a1_a2_b1_60_each", "prior_b2_sequences_1_through_12_exact", "freshness_across_all_prior_urdu_target_ids",
        "record_count", "sequence_13_through_18", "role_cycle", "question_answer_10x10", "bidirectional_links",
    ...
    (ROOT / "reading" / "audit" / "urdu_b2_u03_generation_validation_2026-08-24.json").write_text(json.dumps({
        "schema_version": 1, "project_id": "LANG-A1C2", "language": "ur", "cefr": "B2", "unit": 3,
        "date": DATE, "canonicalized": True, "release_promotion": False,
        "word_counts": [r["word_count"] for r in rows], "checks": checks,
    }, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    (ROOT / "reading" / "audit" / "urdu_b2_u03_promotion_2026-08-24.json").write_text(json.dumps({
        "schema_version": 1, "project_id": "LANG-A1C2", "language": "ur", "cefr": "B2", "unit": 3,
    ...
    lexical_path = ROOT / "reading" / "audit" / "urdu_b2_u03_lexical_sense_check_2026-08-24.json"
    ...
    lexical["freshness_scope"] = "Exact target IDs checked against all canonical Urdu A1-B1 and B2 sequences 1-12 immediately before Unit 3 append."
    lexical_path.write_text(json.dumps(lexical, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")

    status["current"]["canonical_passages"] = 918
    status["current"]["remaining_generation_passages"] = 162
    urdu = status["languages"]["urdu"]
    urdu["generation_state"] = "B2_IN_PROGRESS"
    urdu["canonical_passages"] = 198
    urdu["remaining_generation_passages"] = 162
    urdu["next_generation_level"] = "B2"
    status_path.write_text(json.dumps(status, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")

    continuation["production"]["canonical_passages"] = 918
    cu = continuation["production"]["urdu"]
    cu["state"] = "B2_GENERATION_IN_PROGRESS"
    cu["canonical_passages"] = 198
    cu["next_generation_level"] = "B2"
    continuation["active_frontier"]["production"] = {
        "language": "urdu", "level": "B2",
        "action": "Continue generation-first production from Urdu B2 Unit 4 / sequence 19 using the canonical roadmap and ten-question contract.",
    }
    continuation["exact_next_actions"] = [
        "Validate the routed state bundle and live canonical counts.",
        "Use reading/planning/ACTIVE_GENERATION_PLAN.json to start guarded Urdu B2 Unit 4 generation at sequence 19.",
    ...
    plan["active_unit_roadmap"] = {"unit": 4, "theme": "cities and design", "genres": ["report", "proposal", "critique"]}
    plan_path.write_text(json.dumps(plan, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")

    tasks_path = ROOT / "reading" / "TASKS.md"
    tasks = tasks_path.read_text(encoding="utf-8")
    tasks = replace_once(tasks, "Canonical production frontier: **Urdu B2, Unit 3, sequence 13**.", "Canonical production frontier: **Urdu B2, Unit 4, sequence 19**.", "TASKS frontier")
    tasks = replace_once(tasks, "- Urdu: 192/360 generated; A1-B1 complete, B2 in progress.", "- Urdu: 198/360 generated; A1-B1 complete, B2 in progress.", "TASKS Urdu total")
    tasks = replace_once(tasks, "- Project: 912/1080 generated.", "- Project: 918/1080 generated.", "TASKS project total")
    tasks_path.write_text(tasks, encoding="utf-8")

    handoff_path = ROOT / "reading" / "AGENT_HANDOFF_V2.md"
    handoff = handoff_path.read_text(encoding="utf-8")
    handoff = replace_once(handoff, "- Canonical generated total: **912**.", "- Canonical generated total: **918**.", "handoff total")
    handoff = replace_once(handoff, "- Urdu: **192/360**; A1-B1 generation complete and B2 generation in progress.", "- Urdu: **198/360**; A1-B1 generation complete and B2 generation in progress.", "handoff Urdu")
    handoff = replace_once(handoff, "Continue **Urdu B2**, starting from Unit 3 / sequence 13, under:", "Continue **Urdu B2**, starting from Unit 4 / sequence 19, under:", "handoff frontier")
    handoff = replace_once(handoff, "B2 Unit 3 uses the roadmap theme **ethics and competing values** with `argument`, `case`, and `response` genres.", "B2 Unit 4 uses the roadmap theme **cities and design** with `report`, `proposal`, and `critique` genres.", "handoff roadmap")
    handoff = replace_once(handoff, "resume guarded generation at **Urdu B2 Unit 3 / sequence 13** using the B2 Unit 3 roadmap theme `ethics and competing values`.", "resume guarded generation at **Urdu B2 Unit 4 / sequence 19** using the B2 Unit 4 roadmap theme `cities and design`.", "handoff exact next")
    handoff_path.write_text(handoff, encoding="utf-8")

    print("Urdu B2 Unit 3 canonical promotion prepared: 12 -> 18 B2 records")
    print("Production totals prepared: 912 -> 918 project; Urdu 192 -> 198")
    print("Next frontier prepared: Urdu B2 Unit 4 / sequence 19")
    return 0
