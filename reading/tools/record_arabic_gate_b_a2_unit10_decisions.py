#!/usr/bin/env python3
"""Record fresh Arabic Gate B A2 Unit 10 decisions from the rebuilt Gate B inventory."""
from __future__ import annotations
import hashlib,importlib.util,json
from pathlib import Path
ROOT=Path(__file__).resolve().parents[2]; READING=ROOT/'reading'; CANONICAL=READING/'arabic/a2/passages.jsonl'; INVENTORY=READING/'audit/arabic_gate_b_naturalness_inventory_2026-08-30.json'; DECISION_DIR=READING/'audit/arabic_gate_b_decisions_2026-08-30'; DECISION_PATH=DECISION_DIR/'a2_u10.json'; EXPECTED_IDS=[f'ar-a2-u10-p{i:02d}' for i in range(1,7)]; EXPECTED_FINDINGS=3
def sha(data:bytes)->str:return hashlib.sha256(data).hexdigest()
def load_meta():
 source=Path(__file__).with_name('apply_arabic_gate_b_a2_unit10.py'); spec=importlib.util.spec_from_file_location('arabic_gate_b_a2_u10_meta',source)
 if spec is None or spec.loader is None: raise SystemExit('cannot load A2 Unit 10 repair metadata')
...
def main():
 ...
 if set(meta)!=set(EXPECTED_IDS) or sum(len(meta[p]) for p in EXPECTED_IDS)!=EXPECTED_FINDINGS: raise SystemExit('A2 Unit 10 finding metadata drift')
 decisions=[]
 ...
 doc={'schema_version':1,'project_id':'LANG-A1C2','language':'arabic','level':'A2','unit':10,'date':'2026-09-01','gate':'Gate B — passage-by-passage linguistic/naturalness audit','canonical_path':'reading/arabic/a2/passages.jsonl','canonical_sha256':canonical_sha,'records_reviewed':6,'records_with_findings':sum(1 for d in decisions if d['finding_count']),'fresh_findings':EXPECTED_FINDINGS,'decisions':decisions,'quality_promotion':False,'release_claim':False,'guard':'Learner-facing hashes come only from the freshly rebuilt authoritative Gate B inventory and are independently revalidated by the progress synchronizer.'}
 DECISION_DIR.mkdir(parents=True,exist_ok=True); DECISION_PATH.write_text(json.dumps(doc,ensure_ascii=False,indent=2)+'\n',encoding='utf-8'); print(json.dumps({'level':'A2','unit':10,'records_reviewed':6,'records_with_findings':doc['records_with_findings'],'fresh_findings':EXPECTED_FINDINGS,'canonical_sha256':canonical_sha},ensure_ascii=False,indent=2))
