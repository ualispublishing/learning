...
for r in rows:
 ...
 if r['id'].endswith('p01'):
  ...
  for t in r['new_lexical_targets']:
   if t['id']=='ar-r34': t['part_of_speech']='demonstrative of place / adverbial deictic'
  for q in r.get('questions',[]):
   if q.get('id')=='q3': q['prompt']='ما معنى «هنا» عندما تقول ليلى: «أنا هنا معك»؟'
   elif q.get('id')=='q9': q['prompt']='أكمل بأداة نفي مناسبة: «_____ تخرج ليلى وحدها.»'
  for a in r.get('answer_key',[]):
   if a.get('id')=='a9': a['answer']='لا.'
 elif r['id'].endswith('p02'):
  for t in r.get('new_lexical_targets',[]):
   if t['id']=='ar-r54': t['part_of_speech']='adverb / restrictive expression'
