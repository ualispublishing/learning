...
texts={
'ar-a1-u01-p01':'''ليلى في منزل جديد مع أمها. المنزل صغير، لكنه جميل. قالت أمها: هذا منزلنا الآن. دخلت ليلى وقالت: حسنا، أنا هنا معك. معها حقيبة فيها كتاب. الحقيبة معها في المنزل. قالت أمها: هيا نرى المكان. قالت ليلى: نعم. ذهبتا إلى داخل المنزل ثم إلى خارجه. ثم كانتا داخل المنزل مرة أخرى. قالت ليلى: الآن أعرف المكان. أنا هنا معك الآن.''',
...
p2_prompts=['ماذا قالت الأم قبل الذهاب إلى المدرسة؟','كم كتابا كان مع ليلى؟','ماذا تعني «بعد» في «بعد وقت كانت ليلى في المنزل مرة أخرى»؟','ماذا تعني «فقط» في «كتاب واحد فقط»؟','أكمل: معي كتاب واحد _____، وليس كل الكتب.']
...
for r in rows:
 ...
 if r['id'].endswith('p01'):
  ...
  for q in r.get('questions',[]):
   if q.get('id')=='q3':
    q['prompt']='ما معنى «هنا» عندما تقول ليلى: «أنا هنا معك»؟'
   elif q.get('id')=='q9':
    q['prompt']='أكمل بأداة نفي مناسبة: «_____ تخرج ليلى وحدها.»'
  ...
  for i in range(1,6):
   qid=f'q{i}'; aid=f'a{i}'
   if qid in qmap:
    qmap[qid].update({'type':p2_types[i-1],'prompt':p2_prompts[i-1],'answer_id':aid})
   if aid in amap:
    amap[aid].update({'question_id':qid,'answer':p2_answers[i-1],'explanation':''})
 r['quality']['answer_key_check']='pass'
 r['quality']['notes']=['Recalibrated and exercise-aligned under A1_CALIBRATION_PROFILE; measured coverage pending.','Premature يريد exposure removed from P1-P2 so P3 remains its first deliberate reader introduction.']
