...
claims remain deferred until actual lexical coverage is measured.
"""
...
def main():
 ...
 for level in LEVELS:
  ...
  for r in p6:
   ...
   # Benchmark eligibility cannot pass before coverage/learner calibration, but
   # record whether the corpus consistently keeps it conservative.
   ...
   # Timed status is diagnostic, not mandatory before final calibration.
 ...
 payload={'pass':9,'name':'fluency_checkpoint_suitability','scope':'Arabic A1-C2 P6 passages','method':'unit-role, zero-new-target, timing, comprehension-gate, benchmark-conservatism, and explicit-review diagnostics','not_claimed':['actual learner fluency','known-token coverage','benchmark readiness before calibration'],'levels':summary,'totals':{'p6_passages':sum(x['p6_passages'] for x in summary.values()),'hard_issues':len(hard),'review_flags':len(flags)},'hard_issues':hard,'flags':flags,'status':'FAIL' if hard else ('REVIEW_REQUIRED' if flags else 'PASS')}
