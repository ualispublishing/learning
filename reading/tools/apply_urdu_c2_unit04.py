...
QA = ROOT / "reading/audit/urdu_c2_u04_generation_qa_2026-08-29.json"
SPECS_DIR = ROOT / "reading/tools/c2_unit04_specs"

UNIT = 4
START_SEQUENCE = 19
EXPECTED_EXISTING = 18
EXPECTED_PROJECT = 1038
EXPECTED_URDU = 318
NEW_PROJECT = 1044
NEW_URDU = 324
NEW_REMAINING = 36
NEXT_UNIT = 5
NEXT_SEQUENCE = 25
THEME = "economics and complex systems"
GENRES = ["analytical essay", "scenario analysis", "commentary"]
NEXT_THEME = "literary style and rhetoric"
NEXT_GENRES = ["literary prose", "critical analysis", "rhetorical commentary"]
...
def load_specs():
    ...
    if len(files) != 6:
        raise SystemExit(f"Expected 6 C2 Unit 4 spec files, found {len(files)}")
    return [load_json(p) for p in files]


def make_record(spec, idx):
    seq = START_SEQUENCE + idx
    pid = f"ur-c2-u04-p{idx + 1:02d}"
    ...
    return {
        ...
        "grammar_targets": [{
            "id": "ur-c2-u04-grammar-01",
            "role": "integration",
            "description": "Use conditional, concessive, causal, scope-sensitive, and contrastive structures to distinguish direct effects, feedback, indirect responses, distribution, and temporal change."
        }],
        "discourse_targets": [{
            "id": "ur-c2-u04-discourse-01",
            "role": "integration",
            "description": "Track interacting incentives, feedback loops, second-order effects, coordination failures, institutional inertia, distributional consequences, and shifting evaluation criteria across complex economic scenarios."
        }],
        "questions": questions,
        "answer_key": answers,
        "speed_training": {
        ...
        "quality": {
            ...
            "notes": [
                ...
                "All markets, programs, institutions, prices, and outcomes are explicitly hypothetical instructional constructs rather than claims about real economies or policies."
            ]
        },
        "reader_tags": ["C2", "economics", "complex systems", "systems reasoning", "synthetic scenario"],
...
def validate_existing(existing, cont, status, plan):
    ...
    if [p.get("sequence") for p in existing] != list(range(1, EXPECTED_EXISTING + 1)):
        raise SystemExit("Existing Urdu C2 sequence is not exactly 1..18")
    if existing[-1].get("id") != "ur-c2-u03-p06":
    ...
    if "C2 Unit 4 / sequence 19" not in cont["active_frontier"]["production"]["action"]:
    ...
    if roadmap.get("theme") != THEME or roadmap.get("genres") != GENRES:
        raise SystemExit("ACTIVE_GENERATION_PLAN Unit 4 roadmap drift")
...
def validate_new_records(records):
    ...
    if [r["passage_type"] for r in records] != expected_types:
        raise SystemExit("Unit 4 passage-role cycle drift")
    if [r["sequence"] for r in records] != list(range(19, 25)):
        raise SystemExit("Unit 4 sequences must be 19..24")
    if len({r["id"] for r in records}) != 6:
        raise SystemExit("Duplicate Unit 4 passage ids")
    for r in records:
    ...
    if records[-1]["new_lexical_targets"]:
        raise SystemExit("C2 Unit 4 checkpoint must introduce no new lexical target")
...
def update_state(cont, status, plan):
    ...
    cont["active_frontier"]["production"]["action"] = "Continue generation-first production from Urdu C2 Unit 5 / sequence 25 using the canonical roadmap and ten-question contract."
    cont["exact_next_actions"] = [
        "Validate the routed state bundle and live canonical counts.",
        "Use reading/planning/ACTIVE_GENERATION_PLAN.json to start guarded Urdu C2 Unit 5 generation at sequence 25.",
...
def update_markdown():
    ...
    tasks = replace_required(tasks, "Canonical production frontier: **Urdu C2, Unit 4, sequence 19**.", "Canonical production frontier: **Urdu C2, Unit 5, sequence 25**.", "TASKS frontier")
    tasks = replace_required(tasks, "- Urdu: 318/360 generated; A1-C1 complete, C2 in progress.", "- Urdu: 324/360 generated; A1-C1 complete, C2 in progress.", "TASKS Urdu total")
    tasks = replace_required(tasks, "- Project: 1038/1080 generated.", "- Project: 1044/1080 generated.", "TASKS project total")
    TASKS.write_text(tasks, encoding="utf-8")

    handoff = HANDOFF.read_text(encoding="utf-8")
    handoff = replace_required(handoff, "- Canonical generated total: **1038**.", "- Canonical generated total: **1044**.", "handoff project total")
    handoff = replace_required(handoff, "- Urdu: **318/360**; A1-C1 generation complete and C2 generation in progress.", "- Urdu: **324/360**; A1-C1 generation complete and C2 generation in progress.", "handoff Urdu total")
    handoff = replace_required(handoff, "Continue **Urdu C2**, starting from Unit 4 / sequence 19, under:", "Continue **Urdu C2**, starting from Unit 5 / sequence 25, under:", "handoff frontier")
    handoff = replace_required(handoff, "C2 Unit 4 uses the roadmap theme **economics and complex systems** with `analytical essay`, `scenario analysis`, and `commentary` genres.", "C2 Unit 5 uses the roadmap theme **literary style and rhetoric** with `literary prose`, `critical analysis`, and `rhetorical commentary` genres.", "handoff roadmap")
    handoff = replace_required(handoff, "Run `python reading/tools/validate_continuation_state.py`; if it passes, resume guarded generation at **Urdu C2 Unit 4 / sequence 19** using the C2 Unit 4 roadmap theme `economics and complex systems`.", "Run `python reading/tools/validate_continuation_state.py`; if it passes, resume guarded generation at **Urdu C2 Unit 5 / sequence 25** using the C2 Unit 5 roadmap theme `literary style and rhetoric`.", "handoff exact next action")
...
def main():
    ...
    qa = {
        ...
        "release_claim": False,
        "next_frontier": "Urdu C2 Unit 5 / sequence 25",
        "notes": [
            "All six passages use explicitly hypothetical economic systems, programs, institutions, and outcomes; no factual real-world economic claim is asserted.",
            "Five specialist Unit 4 lexical targets are marked beyond_base rather than assigned invented frequency ranks; the checkpoint introduces no new lexical target.",
    ...
    subprocess.run([sys.executable, "reading/tools/validate_continuation_state.py"], cwd=ROOT, check=True)
    print(json.dumps({"status": "ok", "appended": 6, "urdu_c2_passages": 24, "project_passages": NEW_PROJECT, "next_frontier": "Urdu C2 Unit 5 / sequence 25"}))
