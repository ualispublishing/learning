#!/usr/bin/env python3
"""Resolve canonical French C1 Unit03 planning node after Unit02 lock."""
...
def score(path,node):
 ...
 if re.search(r'unit[_ -]?0?3|u0?3',pl):s+=8
 for k,v in node.items():
  kl=str(k).lower();sv=str(v).strip().lower()
  if kl in {'cefr','level','cefr_level'} and sv=='c1':s+=14
  if kl in {'unit','unit_number','unit_no','unit_id'} and sv in {'3','03','u03','unit03','unit 03','unit 3'}:s+=14
...
def main():
 ...
 if not cands:raise AssertionError('No plausible C1 Unit03 planning node found')
 ...
 if len(top)>1:
  ...
  if conf:raise AssertionError(f'Ambiguous C1 Unit03 planning nodes: {[x["path"] for x in top[:8]]}')
 ...
 if theme is None:raise AssertionError(f'C1 Unit03 node lacks explicit theme/topic/title: {chosen["path"]}')
 out={'status':'PASS','scope':'French C1 Unit03 canonical planning resolution','b2_canonical_blob':b2blob,'c1_source_blob':c1blob,'c1_unit02_lock':'reading/audit/french_c1_unit02_frontier_lock.json','c1_word_min':ready['c1_word_min'],'c1_word_max':ready['c1_word_max'],'matrix_path':chosen['path'],'matrix_score':chosen['score'],'theme':theme,'genres':genres,'domains':domains,'notes':notes,'canonical_node':node,'accepted_c1_default_new_targets_per_standard_passage':lock['accepted_c1_default_new_targets_per_standard_passage'],'accepted_default_is_hard_quota':False,'note':'Resolved from canonical matrix after Unit02 generation seal.'};OUT.parent.mkdir(parents=True,exist_ok=True);OUT.write_text(json.dumps(out,ensure_ascii=False,indent=2)+'\n',encoding='utf-8');print(json.dumps({'status':'PASS','path':chosen['path'],'theme':theme,'genres':genres},ensure_ascii=False))
