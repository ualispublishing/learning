#!/usr/bin/env python3
"""Record fresh Arabic Gate B B2 Unit 6 decisions from the rebuilt Gate B inventory."""
...
DECISION_PATH = DECISION_DIR / "b2_u06.json"
...
def load_meta():
    source = Path(__file__).with_name("apply_arabic_gate_b_b2_unit06.py")
    spec = importlib.util.spec_from_file_location("arabic_gate_b_b2_u06_meta", source)
    if spec is None or spec.loader is None:
        raise SystemExit("cannot load B2 Unit 6 repair metadata")
...
def main() -> None:
    ...
    if len(rows) != 60 or [row.get("sequence") for row in rows] != list(range(1, 61)) or [rows[i].get("id") for i in range(30, 36)] != EXPECTED_IDS:
        raise SystemExit("B2 Unit 6 layout/id drift")

    for record in rows[30:36]:
    ...
    if inventory.get("project_id") != "LANG-A1C2" or inventory.get("language") != "arabic" or (inventory.get("records"), inventory.get("questions"), inventory.get("answers")) != (360, 3600, 3600) or b2.get("canonical_sha256") != canonical_sha:
        raise SystemExit("Gate B B2 inventory identity/scope/hash drift")

    hashes = b2.get("record_learner_facing_sha256", {})
    meta = load_meta()
    if set(meta) != set(EXPECTED_IDS) or sum(len(meta[pid]) for pid in EXPECTED_IDS) != EXPECTED_FINDINGS or sum(bool(meta[pid]) for pid in EXPECTED_IDS) != EXPECTED_WITH_FINDINGS:
        raise SystemExit("B2 Unit 6 finding metadata drift")
    ...
    doc = {
        ...
        "release_claim": False,
        "guard": "Learner-facing hashes come only from the freshly rebuilt authoritative Gate B inventory and are independently revalidated by the progress synchronizer. Legitimate B2 grammar-in-context/discourse analysis is retained; this internal Gate B pass does not constitute educator/publication release approval.",
    }
    DECISION_DIR.mkdir(parents=True, exist_ok=True)
    DECISION_PATH.write_text(json.dumps(doc, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    print(json.dumps({
        "level": "B2", "unit": 6, "records_reviewed": 6,
        "records_with_findings": EXPECTED_WITH_FINDINGS, "fresh_findings": EXPECTED_FINDINGS,
        "canonical_sha256": canonical_sha,
