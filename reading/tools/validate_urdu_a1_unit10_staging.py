...
CANON=ROOT/'reading/urdu/a1/passages.jsonl'; S8=ROOT/'reading/urdu/a1/staging/unit08'; S9=ROOT/'reading/urdu/a1/staging/unit09'; S10=ROOT/'reading/urdu/a1/staging/unit10'; LEX=ROOT/'reading/lexicons/urdu.jsonl'; SEL=ROOT/'reading/audit/urdu_a1_unit10_target_selection_2026-08-21.json'; OUT=ROOT/'reading/audit/urdu_a1_unit10_staging_result_2026-08-21.json'
EXPECTED='b4fcf0bbc07d62cd3e743b8d0a6d49df2d6b0df3d03aa892384d0501a7ef1d4a'
FILES=[f'ur-a1-u10-p{i:02d}.json' for i in range(1,7)]
def wc(s): return len([x for x in str(s).split() if x])
def occ(s,f): return len(re.findall(r'(?<!\w)'+re.escape(str(f))+r'(?!\w)',str(s)))
def load(folder,u): return [json.loads((folder/f'ur-a1-u{u:02d}-p{i:02d}.json').read_text(encoding='utf-8')) for i in range(1,7)]
...
def main():
 ...
 s8=load(S8,8); s9=load(S9,9)
 if len(canon)!=42 or [p.get('sequence') for p in canon]!=list(range(1,43)): raise SystemExit('canonical frontier drift')
 if [p.get('sequence') for p in s8]!=list(range(43,49)) or [p.get('sequence') for p in s9]!=list(range(49,55)): raise SystemExit('prior staged frontier drift')
 h8=agg(s8); h9=agg(s9)
 sel=json.loads(SEL.read_text(encoding='utf-8'))
 if sel.get('bound_canonical_sha256')!=EXPECTED or sel.get('bound_unit08_stage_aggregate_sha256')!=h8 or sel.get('bound_unit09_stage_aggregate_sha256')!=h9: raise SystemExit('selection binding drift')
 selected={x['id']:x for x in sel.get('selected_targets',[])}; selected_forms={x['form'] for x in selected.values()}
 ...
 for seq,name in enumerate(FILES,start=55):
  path=S10/name
  ...
  expected_id=f'ur-a1-u10-p{seq-54:02d}'
  if p.get('id')!=expected_id or p.get('sequence')!=seq or p.get('unit')!=10: problems.append({'file':name,'kind':'identity_sequence_unit'})
  if p.get('language')!='ur' or p.get('cefr')!='A1': problems.append({'file':name,'kind':'language_cefr'})
  text=p.get('text',''); actual_wc=wc(text); actual_sc=text.count('۔')
  ...
  if seq<60 and len(p.get('new_lexical_targets',[]))!=2: problems.append({'file':name,'kind':'expected_two_new_targets'})
  if seq==60:
  ...
  learner=' '.join([p.get('title',''),text]+[q.get('prompt','')+' '+' '.join(q.get('options',[]) or []) for q in qs]+[a.get('answer','')+' '+a.get('explanation','') for a in ans])
 ...
 if problems: raise SystemExit('Unit10 staging validation failed')
