#!/usr/bin/env python3
"""Record fresh Arabic Gate B B2 Unit 5 decisions from the rebuilt Gate B inventory."""
...
DECISION_PATH = DECISION_DIR / "b2_u05.json"
...
def load_meta():
    source = Path(__file__).with_name("apply_arabic_gate_b_b2_unit05.py")
    spec = importlib.util.spec_from_file_location("arabic_gate_b_b2_u05_meta", source)
    if spec is None or spec.loader is None:
        raise SystemExit("cannot load B2 Unit 5 repair metadata")
...
def main() -> None:
    ...
    if len(rows) != 60 or [row.get("sequence") for row in rows] != list(range(1, 61)) or [rows[i].get("id") for i in range(24, 30)] != EXPECTED_IDS:
        raise SystemExit("B2 Unit 5 layout/id drift")

    for record in rows[24:30]:
    ...
    if set(meta) != set(EXPECTED_IDS) or sum(len(meta[pid]) for pid in EXPECTED_IDS) != EXPECTED_FINDINGS or sum(bool(meta[pid]) for pid in EXPECTED_IDS) != EXPECTED_WITH_FINDINGS:
        raise SystemExit("B2 Unit 5 finding metadata drift")
