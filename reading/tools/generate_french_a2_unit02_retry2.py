#!/usr/bin/env python3
"""Retry French A2 Unit 02 with full-batch band and review-visibility repairs."""
...
def build(rows,D):
    ...
    for r in unit:
        ...
        if before!=after:
            raise AssertionError(f"{r['id']}: repairs changed deliberate target counts: {before} -> {after}")
        if not 140<=r['word_count']<=220:
            raise AssertionError(f"{r['id']}: retry outside A2 band at {r['word_count']}")
    return unit
