...
STAGE = ROOT / "reading" / "audit" / "urdu_b2_u04_generation_candidate"
TARGET = ROOT / "reading" / "urdu" / "b2" / "passages.jsonl"
SCHEMA = ROOT / "reading" / "schema" / "passage.schema.json"
DATE = "2026-08-25"

EXPECTED_IDS = [f"ur-b2-u04-p{i:02d}" for i in range(1, 7)]
EXPECTED_SEQ = list(range(19, 25))
EXPECTED_TARGETS = [
    "ur-rank-2397", "ur-rank-2369",
    "ur-rank-2379", "ur-rank-2331",
    "ur-rank-2376", "ur-rank-2294",
    "ur-rank-2400", "ur-rank-2289",
    "ur-rank-2382", "ur-rank-2398",
]
EXPECTED_ROLES = ["instructional", "reinforcement", "interleaved", "transfer", "integration", "checkpoint"]
REQUIRED_GENRES = {"report", "proposal", "critique"}
...
def main() -> int:
    ...
    if len(b2) != 18 or [r.get("sequence") for r in b2] != list(range(1, 19)):
        fail("Urdu B2 must contain exactly sequences 1-18 before Unit 4 promotion")
    ...
    if status["current"]["canonical_passages"] != 918 or status["languages"]["urdu"]["canonical_passages"] != 198:
        fail("STATUS counts are no longer the reviewed B2 Unit 4 pre-promotion state")
    if status["current"]["active_language"] != "urdu" or status["current"]["active_level"] != "B2":
        fail("STATUS active frontier is no longer Urdu B2")
    if plan.get("active_unit") != 4 or plan.get("start_sequence") != 19 or plan.get("existing_active_level_passages") != 18:
        fail("ACTIVE_GENERATION_PLAN no longer points to Urdu B2 Unit 4 / sequence 19")
    action = continuation.get("active_frontier", {}).get("production", {}).get("action", "")
    if "Urdu B2 Unit 4 / sequence 19" not in action:
        fail("CONTINUATION frontier no longer points to Urdu B2 Unit 4 / sequence 19")
    ...
    if collision:
        fail(f"Unit 4 target-ID freshness collision: {collision}")

    canonical_quality = json.loads(json.dumps(b2[-1]["quality"]))
    rows: list[dict] = []
    learner_parts: list[str] = []
    for i in range(1, 7):
        path = STAGE / f"ur-b2-u04-p{i:02d}.json"
        if not path.exists():
    ...
    for row in rows:
        ...
        target_ids = {t["id"] for t in row.get("new_lexical_targets", []) + row.get("review_lexical_targets", [])}
        for target in row.get("new_lexical_targets", []):
        ...
        for q in row["questions"]:
            ...
            unknown_refs = set(q.get("target_ids", [])) - target_ids
            if unknown_refs:
                prior_ids = {t["id"] for r in a1 + a2 + b1 + b2 + rows for t in r.get("new_lexical_targets", [])}
                if not set(q.get("target_ids", [])).issubset(prior_ids):
                    fail(f"question target reference is unknown: {row['id']}/{q['id']}/{sorted(unknown_refs)}")
    ...
    final = jl(TARGET)
    if len(final) != 24 or [r["sequence"] for r in final] != list(range(1, 25)) or [r["id"] for r in final[-6:]] != EXPECTED_IDS:
    ...
    checks = {k: "PASS" for k in [
        ...
        "prior_b2_sequences_1_through_18_exact",
        "freshness_across_all_prior_urdu_target_ids",
        "record_count",
        "sequence_19_through_24",
        "role_cycle",
    ...
    (ROOT / "reading" / "audit" / "urdu_b2_u04_generation_validation_2026-08-25.json").write_text(json.dumps({
        "schema_version": 1,
        "project_id": "LANG-A1C2",
        "language": "ur",
        "cefr": "B2",
    ...
    (ROOT / "reading" / "audit" / "urdu_b2_u04_promotion_2026-08-25.json").write_text(json.dumps({
        "schema_version": 1,
        "project_id": "LANG-A1C2",
        "language": "ur",
        "cefr": "B2",
        ...
        "release_promotion": False,
        "before_record_count": 18,
        "after_record_count": 24,
    ...
    lexical_path = ROOT / "reading" / "audit" / "urdu_b2_u04_lexical_sense_check_2026-08-25.json"
    ...
    lexical["freshness_scope"] = "Exact target IDs checked against all canonical Urdu A1-B1 and B2 sequences 1-18 immediately before Unit 4 append."
    lexical_path.write_text(json.dumps(lexical, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")

    status["current"]["canonical_passages"] = 924
    status["current"]["remaining_generation_passages"] = 156
    urdu_status = status["languages"]["urdu"]
    urdu_status["generation_state"] = "B2_IN_PROGRESS"
    urdu_status["canonical_passages"] = 204
    urdu_status["remaining_generation_passages"] = 156
    urdu_status["next_generation_level"] = "B2"
    status_path.write_text(json.dumps(status, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")

    continuation["updated"] = DATE
    continuation["production"]["canonical_passages"] = 924
    continuation["production"]["urdu"]["canonical_passages"] = 204
    continuation["active_frontier"]["production"]["action"] = "Continue generation-first production from Urdu B2 Unit 5 / sequence 25 using the canonical roadmap and ten-question contract."
    if len(continuation.get("exact_next_actions", [])) < 2:
        fail("CONTINUATION exact_next_actions is unexpectedly short")
    continuation["exact_next_actions"][1] = "Use reading/planning/ACTIVE_GENERATION_PLAN.json to start guarded Urdu B2 Unit 5 generation at sequence 25."
    continuation_path.write_text(json.dumps(continuation, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")

    plan["active_unit"] = 5
    plan["start_sequence"] = 25
    plan["existing_active_level_passages"] = 24
    plan["active_unit_roadmap"] = {
        "unit": 5,
        "theme": "climate and uncertainty",
        "genres": ["evidence summary", "news analysis", "argument"],
    }
    plan_path.write_text(json.dumps(plan, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")

    tasks = tasks_path.read_text(encoding="utf-8")
    tasks = replace_once(tasks, "Updated: 2026-08-24", f"Updated: {DATE}", "TASKS date")
    tasks = replace_once(tasks, "Canonical production frontier: **Urdu B2, Unit 4, sequence 19**.", "Canonical production frontier: **Urdu B2, Unit 5, sequence 25**.", "TASKS frontier")
    tasks = replace_once(tasks, "- Urdu: 198/360 generated; A1-B1 complete, B2 in progress.", "- Urdu: 204/360 generated; A1-B1 complete, B2 in progress.", "TASKS Urdu total")
    tasks = replace_once(tasks, "- Project: 918/1080 generated.", "- Project: 924/1080 generated.", "TASKS project total")
    tasks_path.write_text(tasks, encoding="utf-8")

    handoff = handoff_path.read_text(encoding="utf-8")
    handoff = replace_once(handoff, "Updated: 2026-08-24", f"Updated: {DATE}", "HANDOFF date")
    handoff = replace_once(handoff, "- Canonical generated total: **918**.", "- Canonical generated total: **924**.", "HANDOFF project total")
    handoff = replace_once(handoff, "- Urdu: **198/360**; A1-B1 generation complete and B2 generation in progress.", "- Urdu: **204/360**; A1-B1 generation complete and B2 generation in progress.", "HANDOFF Urdu total")
    handoff = replace_once(handoff, "Continue **Urdu B2**, starting from Unit 4 / sequence 19, under:", "Continue **Urdu B2**, starting from Unit 5 / sequence 25, under:", "HANDOFF frontier")
    handoff = replace_once(handoff, "B2 Unit 4 uses the roadmap theme **cities and design** with `report`, `proposal`, and `critique` genres.", "B2 Unit 5 uses the roadmap theme **climate and uncertainty** with `evidence summary`, `news analysis`, and `argument` genres.", "HANDOFF roadmap")
    handoff = replace_once(handoff, "Run `python reading/tools/validate_continuation_state.py`; if it passes, resume guarded generation at **Urdu B2 Unit 4 / sequence 19** using the B2 Unit 4 roadmap theme `cities and design`.", "Run `python reading/tools/validate_continuation_state.py`; if it passes, resume guarded generation at **Urdu B2 Unit 5 / sequence 25** using the B2 Unit 5 roadmap theme `climate and uncertainty`.", "HANDOFF next action")
    handoff_path.write_text(handoff, encoding="utf-8")

    print("Urdu B2 Unit 4 promotion validation: PASS")
    print("canonical B2 passages: 24; project passages: 924; Urdu passages: 204")
    print("next frontier: Urdu B2 Unit 5 / sequence 25 — climate and uncertainty")
    return 0
