#!/usr/bin/env python3
"""Record fresh Arabic Gate B A1 Unit 5 decisions from the rebuilt Gate B inventory."""
...
DECISION_DIR=READING/'audit/arabic_gate_b_decisions_2026-08-30'; DECISION_PATH=DECISION_DIR/'a1_u05.json'
EXPECTED_IDS=[f'ar-a1-u05-p{i:02d}' for i in range(1,7)]; EXPECTED_FINDINGS=13
def sha(data:bytes)->str:return hashlib.sha256(data).hexdigest()
def load_meta():
 source=Path(__file__).with_name('apply_arabic_gate_b_a1_unit05.py'); spec=importlib.util.spec_from_file_location('arabic_gate_b_a1_u05_meta',source)
 if spec is None or spec.loader is None: raise SystemExit('cannot load Unit 5 repair metadata')
...
def main():
 ...
 if len(rows)!=60 or [r.get('sequence') for r in rows]!=list(range(1,61)): raise SystemExit('A1 layout drift while recording Unit 5 decisions')
 ...
 if set(meta)!=set(EXPECTED_IDS) or sum(len(meta[p]) for p in EXPECTED_IDS)!=EXPECTED_FINDINGS: raise SystemExit('Unit 5 finding metadata drift')
 decisions=[]
 ...
 doc={'schema_version':1,'project_id':'LANG-A1C2','language':'arabic','level':'A1','unit':5,'date':'2026-08-31','gate':'Gate B — passage-by-passage linguistic/naturalness audit','canonical_path':'reading/arabic/a1/passages.jsonl','canonical_sha256':canonical_sha,'records_reviewed':6,'records_with_findings':6,'fresh_findings':EXPECTED_FINDINGS,'decisions':decisions,'quality_promotion':False,'release_claim':False,'guard':'Learner-facing hashes come only from the freshly rebuilt authoritative Gate B inventory and are independently revalidated by the progress synchronizer.'}
 DECISION_DIR.mkdir(parents=True,exist_ok=True); DECISION_PATH.write_text(json.dumps(doc,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
 print(json.dumps({'level':'A1','unit':5,'records_reviewed':6,'fresh_findings':EXPECTED_FINDINGS,'canonical_sha256':canonical_sha,'hash_source':'fresh Gate B inventory'},ensure_ascii=False,indent=2))
