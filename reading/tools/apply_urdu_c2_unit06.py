...
QA = ROOT / "reading/audit/urdu_c2_u06_generation_qa_2026-08-29.json"
SPECS_DIR = ROOT / "reading/tools/c2_unit06_specs"

UNIT = 6
START_SEQUENCE = 31
EXPECTED_EXISTING = 30
EXPECTED_PROJECT = 1050
EXPECTED_URDU = 330
NEW_PROJECT = 1056
NEW_URDU = 336
NEW_REMAINING = 24
NEXT_UNIT = 7
NEXT_SEQUENCE = 37
THEME = "political and institutional argument without partisan advocacy"
GENRES = ["comparative analysis", "policy critique", "argument reconstruction"]
NEXT_THEME = "culture, translation, and interpretation"
NEXT_GENRES = ["essay", "comparative criticism", "reflection"]
...
def load_specs():
    ...
    if len(files) != 6:
        raise SystemExit(f"Expected 6 C2 Unit 6 spec files, found {len(files)}")
    return [load_json(p) for p in files]


def make_record(spec, idx):
    seq = START_SEQUENCE + idx
    pid = f"ur-c2-u06-p{idx + 1:02d}"
    ...
    return {
        ...
        "grammar_targets": [{
            "id": "ur-c2-u06-grammar-01",
            "role": "integration",
            "description": "Use conditional, concessive, comparative, scope-sensitive, and causal structures to reconstruct competing institutional arguments without partisan advocacy."
        }],
        "discourse_targets": [{
            "id": "ur-c2-u06-discourse-01",
            "role": "integration",
            "description": "Track institutional balance, accountability criteria, delegated authority, legitimacy, incentive alignment, counterarguments, trade-offs, and revision mechanisms across hypothetical governance systems."
        }],
        "questions": questions,
        "answer_key": answers,
        "speed_training": {
        ...
        "quality": {
            ...
            "notes": [
                ...
                "All institutions, programs, rules, officials, disputes, and outcomes are hypothetical instructional constructs; no real party, government, election, officeholder, or current political dispute is advocated or characterized."
            ]
        },
        "reader_tags": ["C2", "institutions", "governance", "nonpartisan analysis", "synthetic scenario"],
...
def validate_existing(existing, cont, status, plan):
    ...
    if [p.get("sequence") for p in existing] != list(range(1, EXPECTED_EXISTING + 1)):
        raise SystemExit("Existing Urdu C2 sequence is not exactly 1..30")
    if existing[-1].get("id") != "ur-c2-u05-p06":
    ...
    if "C2 Unit 6 / sequence 31" not in cont["active_frontier"]["production"]["action"]:
    ...
    if roadmap.get("theme") != THEME or roadmap.get("genres") != GENRES:
        raise SystemExit("ACTIVE_GENERATION_PLAN Unit 6 roadmap drift")
...
def validate_new_records(records):
    ...
    if [r["passage_type"] for r in records] != expected_types:
        raise SystemExit("Unit 6 passage-role cycle drift")
    if [r["sequence"] for r in records] != list(range(31, 37)):
        raise SystemExit("Unit 6 sequences must be 31..36")
    if len({r["id"] for r in records}) != 6:
        raise SystemExit("Duplicate Unit 6 passage ids")
    for r in records:
    ...
    if records[-1]["new_lexical_targets"]:
        raise SystemExit("C2 Unit 6 checkpoint must introduce no new lexical target")


def sub_once(text, pattern, repl, label):
    new, n = re.subn(pattern, repl, text, count=1, flags=re.M)
    if n != 1:
        raise SystemExit(f"Could not update {label}; matches={n}")
    return new
...
def update_state(cont, status, plan):
    ...
    cont["active_frontier"]["production"]["action"] = "Continue generation-first production from Urdu C2 Unit 7 / sequence 37 using the canonical roadmap and ten-question contract."
    cont["exact_next_actions"] = [
        "Validate the routed state bundle and live canonical counts.",
        "Use reading/planning/ACTIVE_GENERATION_PLAN.json to start guarded Urdu C2 Unit 7 generation at sequence 37.",
...
def update_markdown():
    ...
    tasks = sub_once(tasks, r"Canonical production frontier: \*\*Urdu C2, Unit 6, sequence 31\*\*\.", "Canonical production frontier: **Urdu C2, Unit 7, sequence 37**.", "TASKS frontier")
    tasks = sub_once(tasks, r"- Urdu: 330/360 generated; A1-C1 complete, C2 in progress\.", "- Urdu: 336/360 generated; A1-C1 complete, C2 in progress.", "TASKS Urdu total")
    tasks = sub_once(tasks, r"- Project: 1050/1080 generated\.", "- Project: 1056/1080 generated.", "TASKS project total")
    TASKS.write_text(tasks, encoding="utf-8")

    handoff = HANDOFF.read_text(encoding="utf-8")
    handoff = sub_once(handoff, r"- Canonical generated total: \*\*1050\*\*\.", "- Canonical generated total: **1056**.", "handoff project total")
    handoff = sub_once(handoff, r"- Urdu: \*\*330/360\*\*; A1-C1 generation complete and C2 generation in progress\.", "- Urdu: **336/360**; A1-C1 generation complete and C2 generation in progress.", "handoff Urdu total")
    handoff = sub_once(handoff, r"Continue \*\*Urdu C2\*\*, starting from Unit 6 / sequence 31, under:", "Continue **Urdu C2**, starting from Unit 7 / sequence 37, under:", "handoff frontier")
    handoff = sub_once(handoff, r"C2 Unit 6 uses the roadmap theme \*\*political and institutional argument without partisan advocacy\*\* with `comparative analysis`, `policy critique`, and `argument reconstruction` genres\.", "C2 Unit 7 uses the roadmap theme **culture, translation, and interpretation** with `essay`, `comparative criticism`, and `reflection` genres.", "handoff roadmap")
    handoff = sub_once(handoff, r"Run `python reading/tools/validate_continuation_state\.py`; if it passes, resume guarded generation at \*\*Urdu C2 Unit 6 / sequence 31\*\* using the C2 Unit 6 roadmap theme `political and institutional argument without partisan advocacy`\.", "Run `python reading/tools/validate_continuation_state.py`; if it passes, resume guarded generation at **Urdu C2 Unit 7 / sequence 37** using the C2 Unit 7 roadmap theme `culture, translation, and interpretation`.", "handoff exact next action")
...
def main():
    ...
    qa = {
        ...
        "release_claim": False,
        "next_frontier": "Urdu C2 Unit 7 / sequence 37",
        "notes": [
            "All six passages use hypothetical institutions and nonpartisan governance scenarios; no real political party, candidate, election, government, or current political controversy is advocated or characterized.",
            "Five specialist Unit 6 lexical targets are marked beyond_base rather than assigned invented frequency ranks; the checkpoint introduces no new lexical target.",
    ...
    subprocess.run([sys.executable, "reading/tools/validate_continuation_state.py"], cwd=ROOT, check=True)
    print(json.dumps({"status":"ok","appended":6,"urdu_c2_passages":36,"project_passages":NEW_PROJECT,"next_frontier":"Urdu C2 Unit 7 / sequence 37"}))
