#!/usr/bin/env python3
"""Record fresh Arabic Gate B B2 Unit 2 decisions from the rebuilt Gate B inventory."""
...
DECISION_PATH = DECISION_DIR / "b2_u02.json"
...
def load_meta():
    source = Path(__file__).with_name("apply_arabic_gate_b_b2_unit02.py")
    spec = importlib.util.spec_from_file_location("arabic_gate_b_b2_u02_meta", source)
    if spec is None or spec.loader is None:
        raise SystemExit("cannot load B2 Unit 2 repair metadata")
...
def main() -> None:
    ...
    if (
        ...
        raise SystemExit("B2 Unit 2 finding metadata drift")
