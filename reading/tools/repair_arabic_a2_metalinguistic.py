...
This script requires all 83 historical before-items to still match current canonical
Q/A after NFC normalization, then applies only their operational replacements.
Passage prose and lexical metadata are protected, and untargeted records remain
byte-identical.
"""
...
def main() -> int:
    ...
    for unit in range(1, 11):
        ...
        source_ledgers.append({"unit": unit, "ref": ref, "artifact": path, "repairs": count})
        for item in repairs:
            repair_specs.append((unit, item))

    if len(repair_specs) != EXPECTED_REPAIRS:
        raise SystemExit(f"Historical A2 ledgers yield {len(repair_specs)} candidates, expected 83")
    ...
    if formal_typed != EXPECTED_FORMAL_TYPED_BEFORE:
        raise SystemExit(f"Expected 82 formal-typed A2 candidates, historical ledgers contain {formal_typed}")
    nonformal_candidates = [
        {"unit": unit, "passage_id": item["passage_id"], "question_id": item["question_id"], "type": item["before"].get("type"), "prompt": item["before"].get("prompt")}
        for unit, item in repair_specs if item["before"].get("type") not in FORMAL_TYPES
    ]
    if len(nonformal_candidates) != 1:
        raise SystemExit(f"Expected exactly one prompt-pattern candidate outside formal types, found {nonformal_candidates}")
    ...
    for unit, item in repair_specs:
        ...
        if nfc(q.get("prompt", "")) != nfc(old.get("prompt", "")) or q.get("type") != old.get("type"):
            raise SystemExit(f"{rid}/{qid}: current question diverges from historical candidate")
        if nfc(ans.get("answer", "")) != nfc(old.get("answer", "")):
            raise SystemExit(f"{rid}/{qid}: current answer diverges from historical candidate")
        ...
        changed.append({
            "unit": unit, "passage_id": rid, "question_id": qid,
            "old_type": old.get("type"), "new_type": new.get("type"),
            "old_prompt": nfc(old.get("prompt", "")), "new_prompt": nfc(new.get("prompt", "")),
    ...
    note = "2026-08-30 A2 low-level metalinguistic remediation: 83 historical candidates re-adjudicated against current canonical Q/A (NFC-equivalent matching) and rewritten as operational A2 tasks; prose/targets preserved."
    ...
    formal_after = [(r["id"], q["id"], q["type"], q.get("prompt")) for r in records for q in r["questions"] if q.get("type") in FORMAL_TYPES]
    if formal_after:
        raise SystemExit(f"Formal question types remain in A2: {formal_after[:10]}")
    ...
    for record in records:
        for q in record["questions"]:
            ...
            if hits:
                explicit_after.append({"passage_id": record["id"], "question_id": q["id"], "type": q.get("type"), "prompt": prompt, "patterns": hits})
    if explicit_after:
        raise SystemExit(f"Explicit formal prompt patterns remain in A2: {explicit_after[:10]}")
    ...
    audit = {
        ...
        "scope": "Current-corpus re-adjudication and bounded Q/A remediation of the complete 83-item historical A2 low-level metalinguistic candidate set.",
        ...
        "nonformal_prompt_pattern_candidate_before": nonformal_candidates,
        "changed_passages": len(changed_record_ids),
        "untargeted_records_byte_identical": True,
        "formal_question_types_after_full_a2": 0,
        ...
        "repairs": changed,
        "quality_interpretation": "Internal substantive Q/A remediation only. This closes the historically identified A2 low-level candidate set under current-corpus guards; CEFR, naturalness, semantic educator, independent native/model/tool, and blind review remain separate release requirements.",
        "release_claim": False,
    }
