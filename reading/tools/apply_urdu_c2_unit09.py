...
QA = ROOT / "reading/audit/urdu_c2_u09_generation_qa_2026-08-30.json"
SPECS_DIR = ROOT / "reading/tools/c2_unit09_specs"

UNIT = 9
START_SEQUENCE = 49
EXPECTED_EXISTING = 48
EXPECTED_PROJECT = 1068
EXPECTED_URDU = 348
NEW_PROJECT = 1074
NEW_URDU = 354
NEW_REMAINING = 6
NEXT_UNIT = 10
NEXT_SEQUENCE = 55
THEME = "technology, ethics, and future uncertainty"
GENRES = ["advanced analysis", "scenario argument", "critique"]
NEXT_THEME = "C2 capstone"
NEXT_GENRES = ["multi-source-style synthesis", "complex paired texts", "final checkpoint"]
...
def load_specs():
    ...
    if len(files) != 6:
        raise SystemExit(f"Expected 6 C2 Unit 9 spec files, found {len(files)}")
    return [load_json(path) for path in files]


def make_record(spec, idx):
    seq = START_SEQUENCE + idx
    pid = f"ur-c2-u09-p{idx + 1:02d}"
    ...
    return {
        ...
        "grammar_targets": [{
            "id": "ur-c2-u09-grammar-01",
            "role": "integration",
            "description": "Use concessive, conditional, counterfactual, causal, comparative, scope-sensitive, and uncertainty-marking structures to analyze technological decisions, ethical trade-offs, and future risk."
        }],
        "discourse_targets": [{
            "id": "ur-c2-u09-discourse-01",
            "role": "integration",
            "description": "Track ethical weighting, reversibility, uncertainty ranges, secondary data use, responsibility attribution, counterarguments, risk distribution, and revision mechanisms across synthetic future-technology scenarios."
        }],
        "questions": questions,
        "answer_key": answers,
        "speed_training": {
        ...
        "quality": {
            ...
            "notes": [
                ...
                "All systems, institutions, technologies, policies, data flows, incidents, and outcomes are original hypothetical instructional constructs; no current factual technology claim is asserted."
            ]
        },
        "reader_tags": ["C2", "technology", "ethics", "future uncertainty", "synthetic scenario"],
...
def validate_existing(existing, cont, status, plan):
    ...
    if [p.get("sequence") for p in existing] != list(range(1, EXPECTED_EXISTING + 1)):
        raise SystemExit("Existing Urdu C2 sequence is not exactly 1..48")
    if existing[-1].get("id") != "ur-c2-u08-p06":
    ...
    if "C2 Unit 9 / sequence 49" not in cont["active_frontier"]["production"]["action"]:
...
def validate_new_records(records):
    ...
    if records[-1]["new_lexical_targets"]:
        raise SystemExit("C2 Unit 9 checkpoint must introduce no new lexical target")
...
def update_state(cont, status, plan):
    ...
    cont["active_frontier"]["production"]["action"] = "Continue generation-first production from Urdu C2 Unit 10 / sequence 55 using the canonical roadmap and ten-question contract."
    cont["exact_next_actions"] = [
        "Validate the routed state bundle and live canonical counts.",
        "Use reading/planning/ACTIVE_GENERATION_PLAN.json to start guarded Urdu C2 Unit 10 capstone generation at sequence 55.",
...
def update_markdown():
    ...
    tasks = replace_required(tasks, "Canonical production frontier: **Urdu C2, Unit 9, sequence 49**.", "Canonical production frontier: **Urdu C2, Unit 10, sequence 55**.", "TASKS frontier")
    tasks = replace_required(tasks, "- Urdu: 348/360 generated; A1-C1 complete, C2 in progress.", "- Urdu: 354/360 generated; A1-C1 complete, C2 in progress.", "TASKS Urdu total")
    tasks = replace_required(tasks, "- Project: 1068/1080 generated.", "- Project: 1074/1080 generated.", "TASKS project total")
    TASKS.write_text(tasks, encoding="utf-8")

    handoff = HANDOFF.read_text(encoding="utf-8")
    handoff = replace_required(handoff, "- Canonical generated total: **1068**.", "- Canonical generated total: **1074**.", "handoff project total")
    handoff = replace_required(handoff, "- Urdu: **348/360**; A1-C1 generation complete and C2 generation in progress.", "- Urdu: **354/360**; A1-C1 generation complete and C2 generation in progress.", "handoff Urdu total")
    handoff = replace_required(handoff, "Continue **Urdu C2**, starting from Unit 9 / sequence 49, under:", "Continue **Urdu C2**, starting from Unit 10 / sequence 55, under:", "handoff frontier")
    handoff = replace_required(handoff, "C2 Unit 9 uses the roadmap theme **technology, ethics, and future uncertainty** with `advanced analysis`, `scenario argument`, and `critique` genres.", "C2 Unit 10 uses the roadmap theme **C2 capstone** with `multi-source-style synthesis`, `complex paired texts`, and `final checkpoint` genres.", "handoff roadmap")
    handoff = replace_required(handoff, "Run `python reading/tools/validate_continuation_state.py`; if it passes, resume guarded generation at **Urdu C2 Unit 9 / sequence 49** using the C2 Unit 9 roadmap theme `technology, ethics, and future uncertainty`.", "Run `python reading/tools/validate_continuation_state.py`; if it passes, resume guarded generation at **Urdu C2 Unit 10 / sequence 55** using the C2 capstone roadmap.", "handoff exact next action")
...
def main():
    ...
    qa = {
        ...
        "release_claim": False,
        "next_frontier": "Urdu C2 Unit 10 / sequence 55",
        "notes": [
            "All six passages use original hypothetical technology, governance, data, forecasting, and automation scenarios; no current factual technology claim is asserted.",
