...
QA = ROOT / "reading/audit/urdu_c2_u05_generation_qa_2026-08-29.json"
SPECS_DIR = ROOT / "reading/tools/c2_unit05_specs"

UNIT = 5
START_SEQUENCE = 25
EXPECTED_EXISTING = 24
EXPECTED_PROJECT = 1044
EXPECTED_URDU = 324
NEW_PROJECT = 1050
NEW_URDU = 330
NEW_REMAINING = 30
NEXT_UNIT = 6
NEXT_SEQUENCE = 31
THEME = "literary style and rhetoric"
GENRES = ["literary prose", "critical analysis", "rhetorical commentary"]
NEXT_THEME = "political and institutional argument without partisan advocacy"
NEXT_GENRES = ["comparative analysis", "policy critique", "argument reconstruction"]
...
def load_specs():
    ...
    if len(files) != 6:
        raise SystemExit(f"Expected 6 C2 Unit 5 spec files, found {len(files)}")
    return [load_json(p) for p in files]


def make_record(spec, idx):
    seq = START_SEQUENCE + idx
    pid = f"ur-c2-u05-p{idx + 1:02d}"
    ...
    return {
        ...
        "grammar_targets": [{
            "id": "ur-c2-u05-grammar-01",
            "role": "integration",
            "description": "Use conditional, concessive, contrastive, parenthetical, and scope-sensitive structures to analyze voice, metaphor, irony, repetition, rhythm, and interpretive alternatives."
        }],
        "discourse_targets": [{
            "id": "ur-c2-u05-discourse-01",
            "role": "integration",
            "description": "Track how metaphorical layering, narrator reliability, rhetorical repetition, ironic distance, rhythmic balance, tone, and syntax interact to shape literary and rhetorical meaning."
        }],
        "questions": questions,
        "answer_key": answers,
        "speed_training": {
        ...
        "quality": {
            ...
            "notes": [
                ...
                "All literary passages, speeches, narrators, institutions, and quoted lines are original hypothetical instructional constructs rather than excerpts from copyrighted or factual sources."
            ]
        },
        "reader_tags": ["C2", "literature", "rhetoric", "style", "original synthetic text"],
...
def validate_existing(existing, cont, status, plan):
    ...
    if [p.get("sequence") for p in existing] != list(range(1, EXPECTED_EXISTING + 1)):
        raise SystemExit("Existing Urdu C2 sequence is not exactly 1..24")
    if existing[-1].get("id") != "ur-c2-u04-p06":
    ...
    if "C2 Unit 5 / sequence 25" not in cont["active_frontier"]["production"]["action"]:
    ...
    if roadmap.get("theme") != THEME or roadmap.get("genres") != GENRES:
        raise SystemExit("ACTIVE_GENERATION_PLAN Unit 5 roadmap drift")
...
def validate_new_records(records):
    ...
    if [r["passage_type"] for r in records] != expected_types:
        raise SystemExit("Unit 5 passage-role cycle drift")
    if [r["sequence"] for r in records] != list(range(25, 31)):
        raise SystemExit("Unit 5 sequences must be 25..30")
    if len({r["id"] for r in records}) != 6:
        raise SystemExit("Duplicate Unit 5 passage ids")
    for r in records:
    ...
    if records[-1]["new_lexical_targets"]:
        raise SystemExit("C2 Unit 5 checkpoint must introduce no new lexical target")
...
def update_state(cont, status, plan):
    ...
    cont["active_frontier"]["production"]["action"] = "Continue generation-first production from Urdu C2 Unit 6 / sequence 31 using the canonical roadmap and ten-question contract."
    cont["exact_next_actions"] = [
        "Validate the routed state bundle and live canonical counts.",
        "Use reading/planning/ACTIVE_GENERATION_PLAN.json to start guarded Urdu C2 Unit 6 generation at sequence 31.",
...
def update_markdown():
    ...
    tasks = replace_required(tasks, "Canonical production frontier: **Urdu C2, Unit 5, sequence 25**.", "Canonical production frontier: **Urdu C2, Unit 6, sequence 31**.", "TASKS frontier")
    tasks = replace_required(tasks, "- Urdu: 324/360 generated; A1-C1 complete, C2 in progress.", "- Urdu: 330/360 generated; A1-C1 complete, C2 in progress.", "TASKS Urdu total")
    tasks = replace_required(tasks, "- Project: 1044/1080 generated.", "- Project: 1050/1080 generated.", "TASKS project total")
    TASKS.write_text(tasks, encoding="utf-8")

    handoff = HANDOFF.read_text(encoding="utf-8")
    handoff = replace_required(handoff, "- Canonical generated total: **1044**.", "- Canonical generated total: **1050**.", "handoff project total")
    handoff = replace_required(handoff, "- Urdu: **324/360**; A1-C1 generation complete and C2 generation in progress.", "- Urdu: **330/360**; A1-C1 generation complete and C2 generation in progress.", "handoff Urdu total")
    handoff = replace_required(handoff, "Continue **Urdu C2**, starting from Unit 5 / sequence 25, under:", "Continue **Urdu C2**, starting from Unit 6 / sequence 31, under:", "handoff frontier")
    handoff = replace_required(handoff, "C2 Unit 5 uses the roadmap theme **literary style and rhetoric** with `literary prose`, `critical analysis`, and `rhetorical commentary` genres.", "C2 Unit 6 uses the roadmap theme **political and institutional argument without partisan advocacy** with `comparative analysis`, `policy critique`, and `argument reconstruction` genres.", "handoff roadmap")
    handoff = replace_required(handoff, "Run `python reading/tools/validate_continuation_state.py`; if it passes, resume guarded generation at **Urdu C2 Unit 5 / sequence 25** using the C2 Unit 5 roadmap theme `literary style and rhetoric`.", "Run `python reading/tools/validate_continuation_state.py`; if it passes, resume guarded generation at **Urdu C2 Unit 6 / sequence 31** using the C2 Unit 6 roadmap theme `political and institutional argument without partisan advocacy`.", "handoff exact next action")
...
def main():
    ...
    qa = {
        ...
        "release_claim": False,
        "next_frontier": "Urdu C2 Unit 6 / sequence 31",
        "notes": [
            "All six passages and quoted lines are original synthetic literary/rhetorical material; no copyrighted excerpt or factual source claim is used.",
            "Five specialist Unit 5 lexical targets are marked beyond_base rather than assigned invented frequency ranks; the checkpoint introduces no new lexical target.",
    ...
    subprocess.run([sys.executable, "reading/tools/validate_continuation_state.py"], cwd=ROOT, check=True)
    print(json.dumps({"status": "ok", "appended": 6, "urdu_c2_passages": 30, "project_passages": NEW_PROJECT, "next_frontier": "Urdu C2 Unit 6 / sequence 31"}))
