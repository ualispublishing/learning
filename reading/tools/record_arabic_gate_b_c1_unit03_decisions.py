#!/usr/bin/env python3
"""Record fresh Arabic Gate B C1 Unit 3 decisions."""
from __future__ import annotations
import hashlib, importlib.util, json
from pathlib import Path
ROOT=Path(__file__).resolve().parents[2]; READING=ROOT/'reading'
CANONICAL=READING/'arabic/c1/passages.jsonl'; INVENTORY=READING/'audit/arabic_gate_b_naturalness_inventory_2026-08-30.json'
DECISION=READING/'audit/arabic_gate_b_decisions_2026-08-30/c1_u03.json'
IDS=[f'ar-c1-u03-p{i:02d}' for i in range(1,7)]; FINDINGS=10; WITH=6
def sha(b): return hashlib.sha256(b).hexdigest()
def meta():
 p=Path(__file__).with_name('apply_arabic_gate_b_c1_unit03.py'); s=importlib.util.spec_from_file_location('c1u03',p); m=importlib.util.module_from_spec(s); s.loader.exec_module(m); return m.FINDING_META
def main():
 raw=CANONICAL.read_bytes(); rows=[json.loads(x) for x in raw.decode().splitlines() if x.strip()]; csha=sha(raw)
 if len(rows)!=60 or [rows[i].get('id') for i in range(12,18)]!=IDS: raise SystemExit('C1 Unit 3 layout/id drift')
 for r in rows[12:18]:
  q=r.get('quality',{})
