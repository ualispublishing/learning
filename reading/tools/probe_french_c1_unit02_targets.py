#!/usr/bin/env python3
"""Exhaustive source/freshness probe for French C1 Unit02 after Unit01 lock."""
...
def main():
 ...
 if plan.get('status')!='PASS' or plan.get('c1_source_blob')!=c1blob or plan.get('b2_canonical_blob')!=b2blob:raise AssertionError('C1 Unit02 plan missing/stale')
 ...
 if len(fresh)<20:raise AssertionError(f'Only {len(fresh)} fresh source terms remain; insufficient for conservative C1 Unit02 production')
 out={'status':'PASS','scope':'French C1 Unit02 exhaustive target probe','b2_canonical_blob':b2blob,'c1_source_blob':c1blob,'c1_unit01_lock':'reading/audit/french_c1_unit01_frontier_lock.json','c1_plan_artifact':'reading/audit/french_c1_unit02_plan.json','c1_theme':plan.get('theme'),'c1_genres':plan.get('genres'),'c1_word_min':plan['c1_word_min'],'c1_word_max':plan['c1_word_max'],'accepted_c1_default_new_targets_per_standard_passage':plan['accepted_c1_default_new_targets_per_standard_passage'],'accepted_default_is_hard_quota':False,'prior_deliberate_targets':len(prior),'deck_entries':len(items),'fresh_count':len(fresh),'fresh':fresh,'already_deliberate_count':len(items)-len(fresh),'note':'Exhaustive source probe for Unit02; generation remains fail-closed behind exact C1 Unit01 lock.'}
 OUT.parent.mkdir(parents=True,exist_ok=True);OUT.write_text(json.dumps(out,ensure_ascii=False,indent=2)+'\n',encoding='utf-8');print(json.dumps({'status':'PASS','fresh_count':len(fresh),'prior_deliberate_targets':len(prior),'theme':plan.get('theme')},ensure_ascii=False))
