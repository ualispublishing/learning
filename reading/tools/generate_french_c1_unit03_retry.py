...
"La crédibilité de l’application compte autant que la sévérité formelle de la sanction. Une règle très dure mais rarement appliquée peut créer moins d’incitation qu’une conséquence plus modeste, prévisible et contrôlable. Le texte distingue donc le barème officiel de la probabilité réelle d’exécution.",
"L’information est également distribuée de façon inégale. Les personnes qui fixent une règle ne voient pas toujours les coûts locaux ni les stratégies de contournement. L’analyse recherche alors des signaux produits par ceux qui exécutent la tâche, sans supposer que leur intérêt particulier rend leur témoignage inutile.",
"Une institution peut enfin apprendre de ses propres erreurs de mesure. Si l’indicateur s’améliore tandis que le résultat recherché stagne, le problème n’est pas nécessairement l’absence d’effort : la mesure peut être devenue une cible en elle-même. La révision porte alors sur le dispositif d’évaluation autant que sur les comportements individuels."
]
orig_fit=ns['fit']
def fit(paras,lo,hi):
