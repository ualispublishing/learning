#!/usr/bin/env python3
"""Record fresh Arabic Gate B C1 Unit 5 decisions."""
from __future__ import annotations
import hashlib, importlib.util, json
from pathlib import Path
ROOT=Path(__file__).resolve().parents[2]; R=ROOT/'reading'; CAN=R/'arabic/c1/passages.jsonl'; INV=R/'audit/arabic_gate_b_naturalness_inventory_2026-08-30.json'; DEC=R/'audit/arabic_gate_b_decisions_2026-08-30/c1_u05.json'
IDS=[f'ar-c1-u05-p{i:02d}' for i in range(1,7)]; FINDINGS=5; WITH=3
def sha(b): return hashlib.sha256(b).hexdigest()
def meta():
 p=Path(__file__).with_name('apply_arabic_gate_b_c1_unit05.py'); s=importlib.util.spec_from_file_location('c1u05',p); m=importlib.util.module_from_spec(s); s.loader.exec_module(m); return m.FINDING_META
def main():
 raw=CAN.read_bytes(); rows=[json.loads(x) for x in raw.decode().splitlines() if x.strip()]; csha=sha(raw)
 if len(rows)!=60 or [rows[i].get('id') for i in range(24,30)]!=IDS: raise SystemExit('C1 Unit 5 layout/id drift')
 for r in rows[24:30]:
  q=r.get('quality',{})
