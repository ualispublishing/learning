#!/usr/bin/env python3
"""Record fresh Arabic Gate B A2 Unit 7 decisions from the rebuilt Gate B inventory."""
from __future__ import annotations
import hashlib,importlib.util,json
from pathlib import Path
ROOT=Path(__file__).resolve().parents[2]; READING=ROOT/'reading'; CANONICAL=READING/'arabic/a2/passages.jsonl'; INVENTORY=READING/'audit/arabic_gate_b_naturalness_inventory_2026-08-30.json'; DECISION_DIR=READING/'audit/arabic_gate_b_decisions_2026-08-30'; DECISION_PATH=DECISION_DIR/'a2_u07.json'; EXPECTED_IDS=[f'ar-a2-u07-p{i:02d}' for i in range(1,7)]; EXPECTED_FINDINGS=10
def sha(data:bytes)->str:return hashlib.sha256(data).hexdigest()
def load_meta():
 source=Path(__file__).with_name('apply_arabic_gate_b_a2_unit07.py'); spec=importlib.util.spec_from_file_location('arabic_gate_b_a2_u07_meta',source)
 if spec is None or spec.loader is None: raise SystemExit('cannot load A2 Unit 7 repair metadata')
...
def main():
 ...
 if set(meta)!=set(EXPECTED_IDS) or sum(len(meta[p]) for p in EXPECTED_IDS)!=EXPECTED_FINDINGS: raise SystemExit('A2 Unit 7 finding metadata drift')
 decisions=[]
