...
- ranks by the validated aggregate frequency;
- merges genuine homographs into one learner-facing front with separate sense blocks;
- keeps extending the ranked candidate stream until there are 1,000 UNIQUE fronts;
...
POS_LABELS = {
    "noun": "noun",
    "noun_num": "numeral noun",
    "noun_quant": "quantifying noun",
...
def clean_gloss(text: str) -> str:
    """Make CALIMA's compact English gloss readable without changing its semantics."""
...
def main() -> None:
    ...
    # Ordered by first (highest-frequency) appearance.  Same Arabic front may have
    # several POS/lexeme analyses; retain those as separate senses on ONE card.
    ...
    for row in candidates:
        ...
        if front not in groups:
            groups[front] = {"front": front, "frequency": 0, "senses": []}
        g = groups[front]
    ...
    # Re-sort by combined homograph frequency; ties are stable by original order.
    ...
    if len({g['front'] for g in selected}) != len(selected):
        problems.append("duplicate learner-facing fronts remain")
    ...
    report_lines = [
        ...
        f"unique_validated_fronts={len(groups)}",
        f"output_rows={len(selected)}",
        f"output_unique_fronts={len({g['front'] for g in selected})}",
        f"source_rows_skipped_bad_front={len(bad)}",
        f"validation_problems={len(problems)}",
        "ranking=aggregate CAMeL MSA frequency after CALIMA-MSA disambiguation; homographs merged by learner-facing front",
