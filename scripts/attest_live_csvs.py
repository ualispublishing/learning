...
def main():
 report={'files':{},'cross_file_checks':{},'problems':[]}
 loaded={}
 for name,expected in FILES.items():
  raw,headers,rows=load(name); loaded[name]=rows
  fronts=[norm(r.get('Front','')) for r in rows]
  file_problems=[]
  ...
  if len(fronts)!=len(set(fronts)): file_problems.append(f'duplicate_normalized_fronts={len(fronts)-len(set(fronts))}')
  ...
  report['files'][name]={
   'rows':len(rows),'expected_rows':expected,'headers':headers,
   'distinct_normalized_fronts':len(set(fronts)),
   'sha256':hashlib.sha256(raw).hexdigest(),
   'bytes':len(raw),'rank_range':rank_range,'problems':file_problems,
   'gate':'PASS' if not file_problems else 'FAIL',
  }
  report['problems'].extend(f'{name}: {p}' for p in file_problems)

 for lang in ('arabic','french','urdu'):
  top={norm(r.get('Front','')) for r in loaded[f'{lang}_top1000.csv']}
  cont={norm(r.get('Front','')) for r in loaded[f'{lang}_top3000.csv']}
  overlap=sorted(top&cont)
  report['cross_file_checks'][f'{lang}_top1000_vs_1001_3000']={
   'overlap_rows':len(overlap),'sample':overlap[:20],
 ...
 report['scope']='Learner-facing root CSVs only; archive/ and audit/ CSVs are evidence, not live decks.'
 out=AUDIT/'live_csv_attestation.json'
