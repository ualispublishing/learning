#!/usr/bin/env python3
"""Single-core-sense Urdu continuation candidate for public release.

Based on v12, with two extra publication safeguards:
- expose only one independently corroborated core meaning per card;
- do not admit ReadUrdu+word2word-only rows when ReadUrdu is just raw legacy
  dictionary notation, unless it explicitly documents the surface as an inflected
  verb form.
"""
...
def make_gloss(word,kg,rg,wg):
 ...
 # Legacy-only ReadUrdu rows are too easy to mis-present; skip them unless a
 # modern Kaikki sense participates.
 ...
 if word in PUBLIC_OVERRIDES:
  ...
  # Override is allowed only when it is still supported somewhere in the same evidence.
...
def main():
 ...
 for raw in top_n_list('ur',250000):
  ...
  if not gloss or len(set(support))<2:
   rej.append({'front':word,'zipf':f'{zipf_frequency(word,"ur"):.2f}','reason':'no_single_clean_core_consensus'});continue
 ...
 with (AUDIT/'urdu_top3000_continuation_evidence_v13.csv').open('w',encoding='utf-8',newline='') as f:w=csv.DictWriter(f,fieldnames=fields);w.writeheader();w.writerows(rows)
 with (AUDIT/'urdu_top3000_candidate_v13.csv').open('w',encoding='utf-8',newline='') as f:
  w=csv.DictWriter(f,fieldnames=['Front','Back']);w.writeheader()
  for r in rows:
   back='\n'.join([f"Rank: {r['rank']}",'',f"Meaning: {r['meaning']}",'',f"Frequency evidence: wordfreq Zipf {r['wordfreq_zipf']}",'Frequency source: wordfreq Urdu multi-corpus ranking','',f"Semantic verification: one core dictionary sense corroborated — {r['semantic_support']}",'','Sources:','- wordfreq Urdu — Unicode multi-corpus frequency ordering','- Kaikki/Wiktextract — bilingual lexicographic evidence','- ReadUrdu — independent Urdu-English bilingual evidence','- word2word — independent corpus-derived bilingual evidence'])
   w.writerow({'Front':r['front'],'Back':back})
 with (AUDIT/'urdu_top3000_rejections_v13.csv').open('w',encoding='utf-8',newline='') as f:fs=['front','zipf','reason'];w=csv.DictWriter(f,fieldnames=fs);w.writeheader();w.writerows(rej)
 bad=[r for r in rows if len(r['meaning'])>45 or RAW.search(r['meaning']) or URDU.search(r['meaning']) or len(WORD.findall(r['meaning']))>3]
 summary={'rows':len(rows),'expected_rows':TARGET,'distinct_fronts':len(seen),'rank_range':[1001,1000+len(rows)] if rows else [],'wordfreq_candidates_examined':examined,'public_gloss_violations':len(bad),'single_core_sense_rows':len(rows),'all_rows_two_source_supported':all(len(set(r['semantic_support'].split('+')))>=2 for r in rows),'structural_gate':'PASS' if len(rows)==TARGET and len(seen)==TARGET and not bad else 'FAIL','status':'candidate_only_not_promoted','policy':'One concise core meaning per card; public gloss originates in a dictionary sense corroborated independently; legacy-only formatted entries are skipped; Unicode wordfreq ordering.'}
 (AUDIT/'urdu_top3000_candidate_v13_summary.json').write_text(json.dumps(summary,ensure_ascii=False,indent=2)+'\n',encoding='utf-8');print(json.dumps(summary,ensure_ascii=False,indent=2))
 if summary['structural_gate']!='PASS':raise SystemExit('v13 gate failed')
