...
check("cissp_atlas_progress_v1" in learner, "learner-state owner no longer uses Atlas card progress")
check("cissp_secx_graph_state_v1" in learner, "graph-specific learner state key missing")
check("const learnerApi=Object.freeze" in learner and "SECX_LEARNER" in learner, "read-only learner API export missing")
check("syncAtlas()" in learner and "atlasRaw" in learner, "learner API no longer resynchronizes same-window storage changes")
check("SECX_LEARNER" in due and "const learner=window.SECX_LEARNER" in due, "Due Reviews no longer consumes the shared learner API")
check("localStorage" not in due, "Due Reviews must not parse/write Atlas storage directly")
check("learner.isDue(c.id)" in due, "due queue no longer uses shared scheduled-due semantics")
