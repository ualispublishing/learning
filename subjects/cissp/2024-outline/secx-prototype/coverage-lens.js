...
window.coverageLayout=function(returnTo=null,focus=false){
  ...
  nodes=[center];links=[];
  domains.forEach((d,i)=>{const m=domainMetrics(d),p=coverageRadialPosition(i,domains.length,.4,.24);nodes.push({id:`coverage:d${d.num}`,domainNum:d.num,title:`D${d.num} · ${d.title||d.name}`,summary:`${m.objectives} objectives · ${m.subtopics} subtopics · ${m.scenarios} scenarios`,kind:'coverage-domain',x:p.x,y:p.y,labels:[`${d.weight}% exam weight`,`${m.subtopics_with_exact_scenario_tag}/${m.subtopics} subtopics tagged`,`${m.objectives_without_released_scenarios} objectives with 0 scenarios`],details:{rule:`Domain ${d.num} explicit coverage summary.`,why:`${m.objectives} objectives, ${m.subtopics} enriched subtopics, ${m.supplemental_cards} supplemental reviewed cards, and ${m.scenarios} released scenarios are explicitly mapped here.`,traps:['Exam weight is blueprint scope, not a learner score.','Scenario count is practice exposure, not proof of completeness.'],sources:['Released Atlas mappings'],practice:'Enter to inspect objective-level counts.'}});links.push([center.id,`coverage:d${d.num}`])});
