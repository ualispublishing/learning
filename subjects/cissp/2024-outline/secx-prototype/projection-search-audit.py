...
check("searchButton.addEventListener('click',()=>{searchOpener=searchButton;openSearch()})" in js,'visible Search control does not preserve opener and open existing palette')
...
if errors:
    ...
    sys.exit(1)
print('PASS secx_projection_search_audit source+coverage navigation=explicit existing-search-index-only combobox-a11y=enabled touch-controls=enabled modal-focus=contained')
