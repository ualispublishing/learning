...
check("function relationshipEndpointLabel(id)" in js and "objectiveById[id]" in js,'reviewed-link search does not derive endpoint labels from the released objective registry')
check("relationshipEndpointLabel(r.from_id)" in js and "relationshipEndpointLabel(r.to_id)" in js,'reviewed-link search does not resolve both stable endpoint labels')
check("${r.from_id} ${fromLabel}" in js and "${r.to_id} ${toLabel}" in js,'reviewed-link searchable text omits human-readable endpoint labels')
...
if errors:
    ...
    sys.exit(1)
print('PASS secx_projection_search_audit source+coverage+reviewed-links navigation=explicit released-relationships-only endpoint-labels=released-objectives existing-search-index-only combobox-a11y=enabled touch-controls=enabled modal-focus=contained visible-open-focus=sync')
