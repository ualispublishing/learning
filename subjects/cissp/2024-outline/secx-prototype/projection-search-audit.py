...
check("searchButton.addEventListener('click',()=>openSearch())" in js,'visible Search control does not open the existing palette')
check("closeButton.addEventListener('click',()=>closeSearch())" in js,'visible Close control does not close the existing palette')
...
if errors:
    ...
    sys.exit(1)
print('PASS secx_projection_search_audit source+coverage navigation=explicit existing-search-index-only combobox-a11y=enabled touch-controls=enabled')
