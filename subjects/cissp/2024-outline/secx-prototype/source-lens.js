...
style.textContent=`
.sec-source-button{border:1px solid #456784;border-radius:999px;background:#10283a;color:var(--text);padding:7px 11px;font:inherit;font-size:11px;cursor:pointer;white-space:nowrap}.sec-source-button:hover,.sec-source-button:focus-visible{border-color:var(--focus);outline:none}.node.sources-root{width:190px;min-height:96px}.node.source-node{width:150px;min-height:70px}.node.source-facet{width:170px;min-height:82px}@media(max-width:800px){.top{gap:6px;padding:8px 10px}.brand{font-size:12px}.sec-source-button,.sec-study-button,.sec-due-button{font-size:9px;padding:5px 7px}.node.sources-root,.node.source-node,.node.source-facet{width:118px;min-height:58px}}
`;
...
window.sourcesLayout=function(returnTo=null,focus=false){
  const ids=sourceIds(),center={id:'sources:root',title:'Source Provenance',summary:`${ids.length} released Atlas sources. Traverse only explicit source_ids mappings.`,kind:'sources-root',x:.5,y:.5,labels:['provenance','explicit mappings'],details:{rule:'Use sources to verify provenance and scope without inventing semantic relationships between co-cited items.',why:'Atlas already records source IDs and source-role notes. This lens exposes those mappings directly.',traps:['Shared citation is not a semantic edge.','A source can support only part of a broader CISSP rule.'],sources:['CISSP Atlas source registry'],practice:'Select a source, then inspect explicitly citing objectives or review cards.'}};
  nodes=[center];links=[];
  ids.forEach((id,i)=>{const p=radialPosition(i,ids.length,.4,.24),m=sourceMeta(id),os=objectivesForSource(id),cs=reviewCardsForSource(id);nodes.push({id:`source:${id}`,sourceId:id,title:m.title||id,summary:`${os.length} objectives · ${cs.length} cards`,kind:'source-node',x:p.x,y:p.y,labels:[id,'released source'],details:sourceDetails(id)});links.push([center.id,`source:${id}`])});
...
window.sourceHubLayout=function(id,returnTo=null,focus=false){
  const m=sourceMeta(id),os=objectivesForSource(id),cs=reviewCardsForSource(id),centerId=`source:${id}`;
  ...
  const facets=[{id:`source:${id}:objectives`,mode:'objectives',title:'Objectives',summary:`${os.length} explicit citations`,x:.27,y:.68},{id:`source:${id}:cards`,mode:'cards',title:'Review cards',summary:`${cs.length} explicit citations`,x:.73,y:.68}];
...
window.sourceItemsLayout=function(id,mode,returnTo=null,focus=false,page=null){
  const m=sourceMeta(id),records=mode==='objectives'?objectivesForSource(id):reviewCardsForSource(id),pages=Math.max(1,Math.ceil(records.length/SOURCE_PAGE_SIZE));
  ...
  nodes=[{id:centerId,sourceId:id,title:`${m.title||id} · ${mode==='objectives'?'Objectives':'Review cards'}`,summary:`${records.length} released items explicitly cite ${id}.`,kind:'sources-root',x:.5,y:.5,labels:[id,mode,'explicit source_ids'],details:{rule:'This is a provenance projection over explicit released source_ids.',why:'No lexical or semantic inference is used to populate this list.',traps:['Co-citation does not imply that items depend on, contrast with, or implement one another.'],sources:[m.url||m.title||id],practice:'Inspect an item, then return to the curriculum graph for its actual hierarchy.'}}];links=[];
  if(slice.length){slice.forEach((item,i)=>{const p=radialPosition(i,slice.length,.39,.24);if(mode==='objectives'){nodes.push({id:`source-item:objective:${item.id}`,itemId:item.id,title:`${item.id} · ${item.label}`,summary:item.summary,kind:'objective',x:p.x,y:p.y,labels:[id,`D${item.domain_num}`,'explicit citation'],details:objectiveDetails(item,id)});links.push([centerId,`source-item:objective:${item.id}`])}else{nodes.push({id:`source-item:card:${item.id}`,itemId:item.id,title:`${item.id} · ${item.topic||item.objective}`,summary:item.front,kind:'card',x:p.x,y:p.y,labels:[id,item.objective,'explicit citation'],details:cardDetails(item,id)});links.push([centerId,`source-item:card:${item.id}`])}});addPager(centerId,sourcePage,pages)}
  else{const empty=`source:empty:${id}:${mode}`;nodes.push({id:empty,title:'No explicit citations',summary:`No released ${mode} currently cite ${id}.`,kind:'due-empty',x:.5,y:.24,labels:[id,'provenance'],details:{rule:'No matching explicit source_ids mapping exists in this view.',why:'The source lens fails empty rather than guessing relationships.',traps:['Do not backfill with text similarity or shared terminology.'],sources:[m.url||m.title||id],practice:'Return to the source hub and choose another branch.'}});links.push([centerId,empty])}
...
window.crumbText=function(){if(level==='sources')return'SecX › Sources';if(level==='source-hub')return`SecX › Sources › ${sourceTitle(sourceId)}`;if(level==='source-items')return`SecX › Sources › ${sourceTitle(sourceId)} › ${sourceMode==='objectives'?'Objectives':'Review cards'}`;return priorCrumb()};
const priorDescend=descend;
window.descend=function(){const n=current();if(level==='sources'&&n?.kind==='source-node')return sourceHubLayout(n.sourceId,null,true);if(level==='source-hub'&&n?.kind==='source-facet')return sourceItemsLayout(n.sourceId,n.mode,null,true,0);if(level==='source-items'){if(n?.kind==='pager')return sourceItemsLayout(sourceId,sourceMode,null,true,sourcePage+(n.action==='next'?1:-1));if(n?.kind==='objective'||n?.kind==='card'||n?.kind==='due-empty'){depth=Math.max(depth,2);return showDetail()}return}return priorDescend()};
