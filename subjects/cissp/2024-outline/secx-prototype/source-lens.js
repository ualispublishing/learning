...
function sourceRadialPosition(i,count,outer,inner){const mobile=window.innerWidth<=800;return radialPosition(i,count,mobile?Math.min(outer,.31):outer,mobile?Math.min(inner,.20):inner)}
...
window.sourcesLayout=function(returnTo=null,focus=false){
  ...
  nodes=[center];links=[];
  ids.forEach((id,i)=>{const p=sourceRadialPosition(i,ids.length,.4,.24),m=sourceMeta(id),os=objectivesForSource(id),cs=reviewCardsForSource(id),qs=scenariosForSource(id);nodes.push({id:`source:${id}`,sourceId:id,title:m.title||id,summary:`${os.length} objectives · ${cs.length} cards · ${qs.length} scenarios`,kind:'source-node',x:p.x,y:p.y,labels:[id,'released source'],details:sourceDetails(id)});links.push([center.id,`source:${id}`])});
...
window.sourceItemsLayout=function(id,mode,returnTo=null,focus=false,page=null){
  ...
  if(slice.length){
    slice.forEach((item,i)=>{
      const p=sourceRadialPosition(i,slice.length,.39,.24);
      if(mode==='objectives'){
...
window.descend=function(){const n=current();if(level==='sources'&&n?.kind==='source-node')return sourceHubLayout(n.sourceId,null,true);if(level==='source-hub'&&n?.kind==='source-facet')return sourceItemsLayout(n.sourceId,n.mode,null,true,0);if(level==='source-items'){if(n?.kind==='pager'){sourceItemsLayout(sourceId,sourceMode,null,false,sourcePage+(n.action==='next'?1:-1));focusActive();return}if(n?.kind==='objective'||n?.kind==='card'||n?.kind==='source-scenario'||n?.kind==='due-empty'){depth=Math.max(depth,2);return showDetail()}return}return priorDescend()};
